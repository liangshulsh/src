    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                string smb = "AAPL,ABMD,ABT,ADSK,ADBE,ADI,ADM,AEP,AFL,HES,AIG,ALK,AMAT,AMD,AME,AMGN,AON,APA,APD,APH,ADP,AXP,AZO,BA,BAC,BAX,BBBY,BBY,BDX,BK,BLL,BMY,BSX,CAG,CAT,C,CAH,CDNS,CERN,CHD,CI,CL,CLX,CMCS_A,CMS,COG,COST,CPB,CSCO,CSX,CTAS,LUMN,CMI,D,DE,DHR,DIS,DHI,DTE,DUK,DVN,ECL,ED,EFX,EMR,EOG,EA,ETN,ETR,F,FAST,M,FDX,FISV,FITB,NEE,GD,GE,INO,GILD,GIS,GLW,GPS,HAL,MNST,HBAN,WELL,PEAK,HD,HOLX,HRL,LHX,HSY,HUM,HPQ,IBM,BIIB,IDXX,IFF,IMMU,INTC,IP,TT,ITW,JNJ,K,KEY,KLAC,KMB,KO,KR,KSS,KSU,LEN,LLY,LOW,LRCX,LB,LUV,MAS,MKC,MCD,MDT,CVS,MGM,SPGI,MMC,MMM,MO,MSI,MRK,MRO,MSFT,MU,MXIM,MYL,NBL,NEM,NKE,JWN,NOC,NSC,ES,ODFL,OKE,OMC,ORCL,OXY,PAYX,PCAR,PCG,PEG,PEP,PFE,PG,PGR,PH,PHM,PNC,PPG,PPL,QCOM,QDEL,RAD,REGN,ROK,ROP,ROST,T,SBUX,SCHW,SHW,SLB,SNPS,SO,TRV,STT,SYK,SWK,NLOK,SYY,TER,TIF,TJX,TMO,TROW,TXN,JCI,TSN,UNH,UNP,RTX,VAR,VFC,VIAC,VLO,VMC,VRTX,WBA,WDC,WEC,WFC,WHR,WMB,WMT,WY,X,XLNX,XOM,FL,CB,INTU,MCHP,ORLY,RCL,BWA,EQR,ATVI,HST,INCY,SPG,MLM,TSCO,ALB,CPRT,PENN,SIRI,O,COF,MCK,DLTR,LMT,DRI,LH,FCX,EL,NTAP,CTXS,HIG,NVAX,VXRT,ALXN,EIX,CHKP,ETFC,DGX,CIEN,KMX,AMTD,TTWO,AMZN,ARE,TFC,SRPT,BXP,MS,PXD,CHRW,NLY,YUM,FE,CAR,URI,VTR,AVB,CTSH,WM,CCI,NVDA,BKNG,GS,SBAC,BMRN,CSGP,AKAM,BLK,PLUG,UPS,TGT,EW,MET,ON,MRVL,ILMN,VZ,XEL,LNG,TPR,EXC,MCO,ALGN,EXAS,GPN,SGEN,MDLZ,WLTW,FIS,ABC,ZBH,ANTM,CVX,AAP,CNC,PRU,JBLU,NFLX,OSTK,SWKS,COP,CNP,DKS,WYNN,CME,EQIX,STX,CCL,A,AMT,SRE,AEE,PLD,EBAY,ALL,STZ,PSA,JPM,USB,HON,ISRG,ACN,MAR,XPO,CRM,DPZ,GOOG_L,DLR,LVS,DXCM,EXPE,ICE,UAA,LYV,CMG,UAL,TDG,MA,HBI,WU,SPR,SRNE,TMUS,DAL,DFS,TEL,BX,LULU,CXO,MELI,VMW,RF,ULTA,MSCI,PM,V,AWK,KDP,AGNC,DISC_A,WKHS,AVGO,VRSK,DG,FTNT,CHTR,LYB,TSLA,KKR,NXPI,GM,FLT,KMI,HCA,YNDX,SAVE,MPC,HZNP,APTV,ZNGA,ENPH,PSX,SPLK,FB,NOW,FIVE,PANW,WDAY,FANG,RH,ABBV,NCLH,ZTS,IQV,CDW,KODK,RNG,BURL,VEEV,WIX,TWTR,CHGG,TNDM,AAL,HLT,ALLY,GRUB,PAYC,ZEN,PE,ANET,INFO,SYF,CZR,CFG,W,HUBS,PLAY,KEYS,QRVO,SHAK,SEDG,GDDY,ETSY,TRU,TDOC,KHC,PYPL,OLLI,LITE,RUN,PLNT,HPE,SQ,TEAM,FTV,TWLO,TTD,COUP,YUMC,AA,INVH,SNAP,AYX,OKTA,CVNA,IR,ATUS,BKR,DD,ROKU,SPCE,MDB,VICI,ZS,DBX,SPOT,DOCU,CDAY,EVRG,NKLA,ELAN,PLAN,STNE,LIN,DELL,MRNA,FOXA,DOW,LYFT,PINS,ZM,SHLL,BYND,UBER,FSLY,CTVA,CRWD,CHWY,WORK,DKNG,LVGO,DT,NET,DDOG,PTON,BILL,OTIS,CARR,MTCH,RKT,SPY";
                //string smb = "CMCSA,VIAB,NYSE:DATA,NYSE:CL,BRKB,NASDAQ:ON,NYSE:A,GOOGL,DISCA";
                string stock_file_long_path = Directory.GetCurrentDirectory() + "\\stocks_long.txt";
                string stock_file_short_path = Directory.GetCurrentDirectory() + "\\stocks_short.txt";
                Console.WriteLine("read stocks short from " + stock_file_short_path);
                //smb = File.ReadAllText(stock_file_short_path);
                Console.WriteLine(smb);
                Console.WriteLine("get stock prices for short");
                string result = dataservice.GetStockBatchQuoteDict(smb, "sina");
                Console.WriteLine(result);
                DateTime curTime = DateTime.Now;
                string path = Directory.GetCurrentDirectory() + "\\stock_price_short_" + curTime.ToString("yyyyMMddhhmmss") + ".txt";
                Console.WriteLine("write stock prices for short to " + path);
                File.WriteAllText(path, result);
                Console.ReadKey();

                Console.WriteLine("read stocks long from " + stock_file_long_path);
                smb = File.ReadAllText(stock_file_long_path);
                Console.WriteLine(smb);
                Console.WriteLine("get stock prices for long");
                result = dataservice.GetStockBatchQuoteDict(smb, "tvc");
                Console.Write(result);
                path = Directory.GetCurrentDirectory() + "\\stock_price_long_" + curTime.ToString("yyyyMMddhhmmss") + ".txt";
                Console.WriteLine("write stock prices for long to " + path);
                File.WriteAllText(path, result);
                Console.ReadKey();
            }
            catch (Exception ex)
