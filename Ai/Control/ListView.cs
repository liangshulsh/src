...
// Created by : Burhanudin Ashari (red_moon@CodeProject) @ August 04 - August 31, 2010(VB Version), April 25, 2011(C# Version).
...
namespace Ai.Control {
	...
	public class ListView : System.Windows.Forms.Control {
		...
		/// <summary>
		/// Occurs when selected item of a ListView Changed.
		/// </summary>
		public event EventHandler<EventArgs> SelectedIndexChanged;
		/// <summary>
		/// Occurs when column header has been reordered.
		/// </summary>
		public event EventHandler<ColumnEventArgs> ColumnOrderChanged;
		/// <summary>
		/// Occurs when filter parameter of a column has been changed.
		/// </summary>
		public event EventHandler<ColumnEventArgs> ColumnFilterChanged;
		/// <summary>
		/// Occurs when custom filter of a column is choosen.
		/// </summary>
        public event EventHandler<ColumnCustomFilterEventArgs> ColumnCustomFilter;
		/// <summary>
		...
		/// </summary>
        public event EventHandler<ColumnBackgroundPaintEventArgs> ColumnBackgroundPaint;
		/// <summary>
		...
		/// </summary>
		public event EventHandler<EventArgs> ItemsClear;
		/// <summary>
		/// Occurs when mouse pointer is hover on a ListViewItem object.
		/// </summary>
        public event EventHandler<ItemEventArgs> ItemHover;
		/// <summary>
		/// Occurs before checked state of a ListViewItem changed.
		/// </summary>
        public event EventHandler<ItemEventArgs> ItemBeforeCheck;
		/// <summary>
		/// Occurs after checked state of a ListViewItem changed.
		/// </summary>
        public event EventHandler<ItemEventArgs> ItemAfterCheck;
		/// <summary>
		/// Occurs when the user starts editing the label of an item.
		/// </summary>
        public event EventHandler<ItemEventArgs> BeforeLabelEdit;
		/// <summary>
		/// Occurs when the label for an item is edited by the user.
		/// </summary>
        public event EventHandler<ItemEventArgs> AfterLabelEdit;
		/// <summary>
		/// Occurs before the default group is collapsed.
		/// </summary>
        public event EventHandler<GroupEventArgs> DefaultGroupBeforeCollapse;
		/// <summary>
		/// Occurs after the default group is collapsed.
		/// </summary>
        public event EventHandler<GroupEventArgs> DefaultGroupAfterCollape;
		/// <summary>
		/// Occurs before the default group is expanded.
		/// </summary>
        public event EventHandler<GroupEventArgs> DefaultGroupBeforeExpand;
		/// <summary>
		/// Occurs after the default group is expanded.
		/// </summary>
        public event EventHandler<GroupEventArgs> DefaultGroupAfterExpand;
		/// <summary>
		/// Occurs before checked state of the default group is changed.
		/// </summary>
        public event EventHandler<GroupEventArgs> DefaultGroupBeforeCheck;
		/// <summary>
		/// Occurs after checked state of the default group is changed.
		/// </summary>
        public event EventHandler<GroupEventArgs> DefaultGroupAfterCheck;
        // Drag and Drop events.
        /// <summary>
        /// Occurs when node(s) is started to drags.
        /// </summary>
        public event EventHandler<ItemDragDropEventArgs> ItemDragStart;
        /// <summary>
        /// Occurs when the drag operation is moved.
        /// </summary>
        public event EventHandler<ItemDragDropEventArgs> ItemDragMove;
        /// <summary>
        /// Occurs when the drag operation is dropped.
        /// </summary>
        public event EventHandler<ItemDragDropEventArgs> ItemDragDrop;
        /// <summary>
        /// Occurs when the drag operation is leaves the control.
        /// </summary>
        public event EventHandler<ItemDragDropEventArgs> ItemDragLeave;
        /// <summary>
        /// Occurs when the ghost image of the drag operation needs to be drawn.
        /// </summary>
        public event EventHandler<PaintEventArgs> DrawDragBitmap;
		#endregion
		...
		public class ListViewItemCollection : CollectionBase {
			...
			#region Constructor
			public ListViewItemCollection(ListView owner) : base() { _owner = owner; }
			#endregion
			#region Public Events
			...
			#endregion
			/// <summary>
			/// Gets a ListViewItem object in the collection specified by its index.
			/// </summary>
			[Description("Gets a ListViewItem object in the collection specified by its index.")]
			public ListViewItem this[int index] {
				get {
					if (index >= 0 && index < List.Count) return (ListViewItem)List[index];
					return null;
				}
			}
			#region Public Functions
			/// <summary>
			/// Gets the index of a ListViewItem object in the collection.
			/// </summary>
			[Description("Gets the index of a ListViewItem object in the collection.")]
			public int IndexOf(ListViewItem item) { return this.List.IndexOf(item); }
			/// <summary>
			/// Add a ListViewItem object to the collection.
			/// </summary>
			...
			/// <summary>
			/// Add a ListViewItem object to the collection by providing its text.
			/// </summary>
			...
			/// <summary>
			/// Add a ListViewItem collection to the collection.
			/// </summary>
			...
			public void AddRange(ListViewItemCollection items) {
				foreach (ListViewItem item in items) this.Add(item);
			}
			/// <summary>
			/// Insert a ListViewItem object to the collection at specified index.
			/// </summary>
			...
			/// <summary>
			/// Remove a ListViewItem object from the collection.
			/// </summary>
			...
			/// <summary>
			/// Determine whether a ListViewItem object exist in the collection.
			/// </summary>
			[Description("Determine whether a ListViewItem object exist in the collection.")]
			public bool Contains(ListViewItem item) { return List.Contains(item); }
			#endregion
			#region Private Functions
			private bool containsName(string name) {
				foreach (ListViewItem lvi in List) {
					if (string.Compare(lvi.Name, name, true) == 0) return true;
				}
				return false;
			}
			#endregion
			...
			protected override void OnValidate(object value) {
				if (!typeof(ListViewItem).IsAssignableFrom(value.GetType())) throw new ArgumentException("Value must Ai.Control.ListViewItem", "value");
			}
		...
		public class FrozenListViewItemCollection : CollectionBase {
			internal ListView _owner;
            #region Constructor
            public FrozenListViewItemCollection(ListView owner) : base() { _owner = owner; }
			#endregion
			#region Public Events
            public event EventHandler<CollectionEventArgs> Clearing;
            public event EventHandler<CollectionEventArgs> AfterClear;
			...
			public void Insert(int index, ListViewItem item) {
				...
				if (!this.Contains(item)) {
					// Only adding the existing item in ListView.
					if (_owner._items.Contains(item)) List.Insert(index, item);
				}
			}
            /// <summary>
            /// Remove a ListViewItem object from the collection.
		...
		public class CheckedListViewItemCollection : CollectionBase {
			internal ListView _owner;
            #region Constructor
            public CheckedListViewItemCollection(ListView owner) : base() { _owner = owner; }
			#endregion
			#region Public Events
            public event EventHandler<CollectionEventArgs> Clearing;
            public event EventHandler<CollectionEventArgs> AfterClear;
        ...
        /// <summary>
        /// Represent a collection of selected ListViewItem objects in a ListView or ListViewGroup.
        /// </summary>
        [Description("Represent a collection of selected ListViewItem objects in a ListView or ListViewGroup.")]
        public class SelectedListViewItemCollection : CollectionBase {
            internal ListView _owner;
            public SelectedListViewItemCollection(ListView owner) : base() { _owner = owner; }
            #region Public Events
            public event EventHandler<CollectionEventArgs> Clearing;
            public event EventHandler<CollectionEventArgs> AfterClear;
            public event EventHandler<CollectionEventArgs> Inserting;
		...
		public class ColumnHeaderCollection : CollectionBase {
			internal ListView _owner;
            #region Constructor
            public ColumnHeaderCollection(ListView owner) : base() { _owner = owner; }
			#endregion
			#region Public Events
            public event EventHandler<CollectionEventArgs> Clearing;
            public event EventHandler<CollectionEventArgs> AfterClear;
			...
			#region Public Functions
            /// <summary>
            /// Gets a ColumnHeader object in the collection specified by its index.
            /// </summary>
            [Description("Gets a ColumnHeader object in the collection specified by its index.")]
			public ColumnHeader this[int index] {
				get {
					if (index >= 0 & index < List.Count) return (ColumnHeader)List[index];
					return null;
				}
			}
            /// <summary>
            /// Gets the index of a ColumnHeader object in the collection.
            /// </summary>
            [Description("Gets the index of a ColumnHeader object in the collection.")]
			public int IndexOf(ColumnHeader item) { return this.List.IndexOf(item); }
            /// <summary>
            /// Add a ColumnHeader object to the collection.
            /// </summary>
            ...
            /// <summary>
            /// Add a ColumnHeader object to the collection by providing its text.
            /// </summary>
            ...
            /// <summary>
            /// Add a ColumnHeader collection to the collection.
            /// </summary>
            [Description("Add a ColumnHeader collection to the collection."), 
			    System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1062:ValidateArgumentsOfPublicMethods")]
			public void AddRange(ColumnHeaderCollection columns) {
				foreach (ColumnHeader column in columns) this.Add(column);
			}
            /// <summary>
            /// Insert a ColumnHeader object to the collection at specified index.
            /// </summary>
            ...
            /// <summary>
            /// Remove a ColumnHeader object from the collection.
            /// </summary>
            ...
            /// <summary>
            /// Determine whether a ColumnHeader object exist in the collection.
            /// </summary>
            [Description("Determine whether a ColumnHeader object exist in the collection.")]
			public bool Contains(ColumnHeader header) { return List.Contains(header); }
			#endregion
			#region Private Functions
			private bool containsName(string name) {
				foreach (ColumnHeader ch in List) {
					if (string.Compare(ch.Name, name, true) == 0) return true;
				}
				return false;
			}
			#endregion
			...
			[Description("Performs additional custom processes when validating a value.")]
			protected override void OnValidate(object value) {
				if (!typeof(ColumnHeader).IsAssignableFrom(value.GetType())) throw new ArgumentException("Value must Ai.Control.ColumnHeader", "value");
			}
			protected override void OnClear() {
			...
			protected override void OnInsertComplete(int index, object value) {
				_owner._columnControl.addHost((ColumnHeader)value);
				_owner._columnControl.Invalidate();
				if (AfterInsert != null) AfterInsert(this, new CollectionEventArgs(CollectionEventArgs.EventType.OnInsertComplete, index, value));
			}
			protected override void OnRemove(int index, object value) {
			...
			private void columnAppearanceChanged(object sender, EventArgs e) { _owner._columnControl.Invalidate(); }
				...
				if (ch.EnableSorting) {
					if (ch.SortOrder != SortOrder.None) {
						foreach (ColumnHeader otherCH in List) {
							if (otherCH != ch) otherCH.SortOrder = SortOrder.None;
						}
						_owner._columnRef = _owner._columns.IndexOf(ch);
						_owner.sortAll();
						_owner.relocateAll();
						if (_owner._view == Control.View.Preview) _owner.drawPreviewHosts();
						_owner.Invalidate();
					}
				}
			}
			private void columnItemsRelatedValueChanged(object sender, EventArgs e) {
				if (_owner._view == View.Details || _owner._view == View.Tile) _owner.Invalidate();
			}
        ...
        /// <summary>
        /// The size of the dragged bitmap.
        /// </summary>
        const int _dragBmpSize = 80;
        #endregion
        #region Members.
        // Components
        SelectedListViewItemCollection _selectedItems;
        CheckedListViewItemCollection _checkedItems;
        ListViewItemCollection _items;
        ListViewGroupCollection _groups;
        ColumnHeaderCollection _columns;
        FrozenListViewItemCollection _frozenItems;
        VScrollBar _vScroll = new VScrollBar();
        HScrollBar _hScroll = new HScrollBar();
        // Properties
        ...
        bool _customDrawDragBitmap = false;
        // Internal use
        /// <summary>
        /// Custom Control to handle all column header operations.
        /// </summary>
        ColumnHeaderControl _columnControl;
        /// <summary>
        ...
        // Drag drop operation.
        /// <summary>
        /// Indicating whether the user starts drag operation.
        /// </summary>
        bool _dragBegin = false;
        /// <summary>
        /// A point to store the first location of drag operation.
        /// </summary>
        Point _startDragPoint = new Point();
        /// <summary>
        /// Image list to store the ghost image of drag operation.
        /// </summary>
        ImageList _imglDrag = new ImageList();
        /// <summary>
        /// A MouseButtons to store information about mouse button used to drag.
        /// </summary>
        int _dragKeyState = 0;
        /// <summary>
        /// A timer object to scroll up or down tree view when user drags the mouse inside 20 pixel above the bottom border or below the column header.
        /// </summary>
        Timer _timer = new Timer();
        /// <summary>
        /// An object to store the source of the drag operation.
        /// </summary>
        object _dragSource = null;
        /// <summary>
        ...
        /// Host of hovers node.
        /// </summary>
        ListViewItemHost _hoverHost = null;
        // Group Host
        /// <summary>
        ...
        /// </summary>
        private Bitmap _gBmp = new Bitmap(1, 1);
        /// <summary>
        ...
        #endregion
        #region Core Engines.  Classes to handle all objects, key and mouse event handlers, and as a visual representation of every object associated with ListView.
	    /// <summary>
	    ...
	    private class ColumnHeaderControl : System.Windows.Forms.Control {
		    ...
		    private class ColumnHost {
			    ...
			    public void draw(Graphics g) {
				    ...
				    if (_column.Image != null) {
					    ...
					    imgColRect.Size = Ai.Renderer.Drawing.scaleImage(_column.Image, 16);
					    imgColRect.Y = (int)((_rect.Height - imgColRect.Height) / 2);
				    ...
				    if (_column.SortOrder != SortOrder.None && _column.EnableSorting) txtColRect.Width = txtColRect.Width - 9;
				    ...
				    if (_owner.Enabled) {
					    ...
					    if (_onHover) {
						    ...
						    if (_column.EnableFiltering) {
							    ...
							    if (_onMouseDownSplit) {
								    ...
								    if (_onHoverSplit) splitBrush.InterpolationColors = Ai.Renderer.Column.HLitedDropDownBlend();
								    else splitBrush.InterpolationColors = Ai.Renderer.Column.HLitedBlend();
							    }
						    }
						    if (_onMouseDown) bgBrush.InterpolationColors = Ai.Renderer.Column.PressedBlend();
					    ...
					    if (_column.EnableFiltering) {
						    ...
						    Ai.Renderer.Drawing.drawTriangle(g, _rect.Right - 8, _rect.Y + (int)((_rect.Height - 6) / 2), Color.FromArgb(21, 66, 139), Color.White, Ai.Renderer.Drawing.TriangleDirection.Down);
					    }
				    } else {
					    if (_column.EnableFiltering) {
						    g.DrawLine(Pens.Gray, _rect.Right - 10, _rect.Y + 1, _rect.Right - 10, _rect.Bottom - 2);
						    Ai.Renderer.Drawing.drawTriangle(g, _rect.Right - 8, _rect.Y + (int)((_rect.Height - 6) / 2), Color.Gray, Color.White, Ai.Renderer.Drawing.TriangleDirection.Down);
					    }
				    }
				    if (_column.Image != null) {
					    if (_owner.Enabled) g.DrawImage(_column.Image, imgColRect);
					    else Ai.Renderer.Drawing.grayScaledImage(_column.Image, imgColRect, g);
				    }
				    g.DrawString(_column.Text, _owner.Font, (_owner.Enabled ? Ai.Renderer.Column.TextBrush() : Ai.Renderer.Drawing.DisabledTextBrush()), txtColRect, txtColFormat);
				    if (_column.EnableSorting) {
					    switch (_column.SortOrder) {
						    case SortOrder.Ascending:
							    Ai.Renderer.Drawing.drawTriangle(g, sortSignLeft, _rect.Y + (int)((_rect.Height - 6) / 2), (_owner.Enabled ? Color.FromArgb(21, 66, 139) : Color.Gray), Color.White, Ai.Renderer.Drawing.TriangleDirection.Up);
							    break;
						    case SortOrder.Descending:
							    Ai.Renderer.Drawing.drawTriangle(g, sortSignLeft, _rect.Y + (int)((_rect.Height - 6) / 2), (_owner.Enabled ? Color.FromArgb(21, 66, 139) : Color.Gray), Color.White, Ai.Renderer.Drawing.TriangleDirection.Down);
							    break;
					    }
				    }
				    if (_onMouseDown) Ai.Renderer.Column.drawPressedShadow(g, _rect);
			    }
			    /// <summary>
			    ...
			    /// <param name="canDrop">Determine whether the column can be dropped on a location, default = true.</param>
			    public void drawMoved(Graphics g, Rectangle rect, bool canDrop) {
				    ...
				    if (_column.EnableFiltering) txtColRect.Width = txtColRect.Width - 11;
				    if (_column.SortOrder != SortOrder.None && _column.EnableSorting) txtColRect.Width = txtColRect.Width - 9;
				    txtColFormat.LineAlignment = StringAlignment.Center;
				    switch (_column.TextAlign) {
					    case HorizontalAlignment.Center:
				    ...
				    if (!canDrop) {
					    ...
					    g.DrawLine(new Pen(Color.White, 3), rectSign.X + 2, rectSign.Y + (int)(rectSign.Height / 2), rectSign.Right - 3, rectSign.Y + (int)(rectSign.Height / 2));
				    }
			    }
                public void drawMoved(Graphics g, Rectangle rect) { drawMoved(g, rect, true); }
			    /// <summary>
			    ...
			    public bool mouseMove(MouseEventArgs e) {
				    ...
				    if (_column.EnableFiltering) rectSplit = new Rectangle(_rect.Right - 10, _rect.Y, 10, _rect.Height);
				    if (_rect.Contains(e.Location)) {
					    ...
					    if (!_onHover) {
						    ...
						    if (Ai.Renderer.ToolTip.containsToolTip(_column.ToolTipTitle, _column.ToolTip, _column.ToolTipImage)) {
							    ...
							    if (needToolTip()) _owner._currentToolTip = _column.Text;
						    }
						    stateChanged = true;
					    }
				    } else {
			    ...
			    public bool mouseDown() {
				    if (_onHover || _onHoverSplit) {
					    if (_owner._selectedHost != null) _owner._selectedHost._selected = false;
					    _selected = true;
					    ...
					    if (_onHoverSplit) {
						    ...
						    _owner.showFilterPopup(new FilterChooser(_filterHandler, _owner._toolStrip, Ai.Renderer.ToolTip.TextFont, _owner._owner._ci), rectSplit);
					    } else {
						    _onMouseDown = true;
			    ...
			    public void addItemToFilter(ListViewItem item) {
				    if (_column.EnableFiltering) {
					    ...
					    if (item.SubItems[columnIndex] != null) _filterHandler.addFilter(item.SubItems[columnIndex].Value);
				    }
			    }
			    /// <summary>
			    /// Clear all filter parameter on the column.
			    /// </summary>
			    public void clearFilter() { _filterHandler.Items.Clear(); }
			    /// <summary>
			    ...
			    public int Width {
				    ...
				    set {
					    if (_rect.Width != value) {
						    if (value >= MinResize) _rect.Width = value;
						    else _rect.Width = MinResize;
					    }
				    }
			    }
			    /// <summary>
			    ...
			    /// </summary>
			    public bool OnMouseDown { get { return _onMouseDown; } }
			    /// <summary>
			    /// Gets a value indicating the split column has been pressed.
			    /// </summary>
			    public bool OnMouseDownSplit { get { return _onMouseDownSplit; } }
			    /// <summary>
			    /// Gets a value indicating the mouse pointer moved over the column.
			    /// </summary>
			    public bool OnHover { get { return _onHover; } }
			    /// <summary>
			    /// Gets a value indicating the mouse pointer moved over the column's split.
			    /// </summary>
			    public bool OnHoverSplit { get { return _onHoverSplit; } }
			    /// <summary>
			    ...
			    public int MinResize {
				    get {
					    int minWidth = 10;
					    if (_column.Image != null) minWidth = minWidth + 18;
					    if (_column.EnableFiltering && _column.SortOrder != SortOrder.None) minWidth = minWidth + 10;
					    if (_column.EnableFiltering) minWidth = minWidth + 10;
			    ...
			    /// </summary>
			    public ColumnFilterHandle FilterHandler { get { return _filterHandler; } }
		    }
		    /// <summary>
		    ...
		    public event EventHandler<ColumnEventArgs> AfterColumnFilter;
            /// <summary>
            /// Occurs when the column custom filter is choosen.
            /// </summary>
		    [Description("Occurs when the column custom filter is choosen.")]
		    public event EventHandler<ColumnEventArgs> AfterColumnCustomFilter;
            /// <summary>
		    ...
		    public event EventHandler<ColumnEventArgs> AfterColumnResize;
		    ...
		    public void removeHost(ColumnHeader aColumn) {
			    int i = 0;
			    if (_selectedHost.Column == aColumn) _selectedHost = null;
			    while (i < _columnHosts.Count) {
				    if (object.ReferenceEquals(_columnHosts[i].Column, aColumn)) _columnHosts.RemoveAt(i);
				    else i++;
			    }
			    foreach (ColumnHost ch in _columnHosts) ch.Column._displayIndex = _columnHosts.IndexOf(ch);
			    relocateHosts();
		    }
		    /// <summary>
		    ...
		    /// </summary>
		    /// <param name="startX">Optional, determine the starting x location of the host of unfrozen columns, default = 0.</param>
		    public void relocateHosts(int startX) {
			    ...
			    if (_owner._view != Control.View.Preview) availWidth -= _owner._vScroll.Width;
			    ...
			    if (availWidth > 0) {
					    ...
					    if (ch.Column.Frozen && ch.Column.Visible) {
						    switch (ch.Column.SizeType) {
							    ...
							    case ColumnSizeType.Percentage:
								    ch.Width = (int)(ch.Column.Width * availWidth / 100);
								    break;
						    }
				    ...
				    foreach (ColumnHost ch in _columnHosts) {
					    if (ch.Column.Visible && !ch.Column.Frozen) {
						    ...
						    ch.Height = this.Height;
						    if (ch.Column.SizeType != ColumnSizeType.Fill) availWidth = availWidth - (ch.Width + 2);
					    }
				    }
				    if (fillSizeCount > 0) {
					    foreach (ColumnHost ch in _columnHosts) {
						    if (ch.Column.Visible && ch.Column.SizeType == ColumnSizeType.Fill) ch.Width = (int)((availWidth + (2 * fillSizeCount)) / fillSizeCount);
					    }
				    }
			    }
            ...
            public void relocateHosts() { relocateHosts(0); }
		    /// <summary>
		    ...
		    public bool OnColumnResize { get { return _onColumnResize; } }
		    /// <summary>
		    /// Gets current column resize position.
		    /// </summary>
		    [Description("Gets current column resize position.")]
		    public int ResizeCurrentX { get { return _resizeCurrentX; } }
		    /// <summary>
		    ...
		    public int ColumnsWidth {
			    get {
				    int result = 0;
				    foreach (ColumnHost ch in _columnHosts) {
					    if (ch.Column.Visible) result = result + ch.Width + 2;
				    }
				    return result;
			    }
		    }
		    ...
		    public ColumnHeader ColumnAt(int index) {
			    if (index >= 0 & index < _columnHosts.Count) return _columnHosts[index].Column;
				return null;
		    }
		    /// <summary>
		    ...
		    public Rectangle ColumnRectangle(ColumnHeader column) {
		        foreach (ColumnHost ch in _columnHosts) {
				    if (ch.Column == column) return ch.Bounds;
			    }
		    ...
		    public int UnFrozenWidth {
			    get {
				    int result = 0;
				    foreach (ColumnHost ch in _columnHosts) {
					    if (ch.Column.Visible && !ch.Column.Frozen) result = result + ch.Width + 2;
				    }
				    return result;
			    }
		    }
		    ...
		    public List<ColumnHeader> FrozenColumns {
			    get {
				    ...
				    foreach (ColumnHost ch in _columnHosts) {
					    if (ch.Column.Visible & ch.Column.Frozen) result.Add(ch.Column);
				    }
				    return result;
			    }
		    }
				    ...
				    foreach (ColumnHost ch in _columnHosts) {
					    if (ch.Column.Visible) {
						    if (first) {
							    ...
							    if (leftMost > ch.Bounds.X) leftMost = ch.Bounds.X;
						    }
						    if (rightMost < ch.Bounds.Right + 2) rightMost = ch.Bounds.Right + 2;
					    }
				    }
		    ...
		    public ColumnFilterHandle FilterHandler(ColumnHeader column) {
		        foreach (ColumnHost ch in _columnHosts) {
				    if (ch.Column == column) return ch.FilterHandler;
			    }
			    return null;
		    }
		    ...
		    public void clearFilters() {
			    foreach (ColumnHost ch in _columnHosts) ch.FilterHandler.Items.Clear();
		    }
		    /// <summary>
		    ...
		    public int getDisplayedIndex(ColumnHeader column) {
			    ...
			    foreach (ColumnHost colHost in _columnHosts) {
				    if (colHost.Column.Visible && colHost.Column.Frozen) {
					    if (colHost.Column == column) return index;
					    index += 1;
				    }
			    }
		    ...
		    private void ColumnHeaderControl_DoubleClick(object sender, System.EventArgs e) {
			    ...
			    if (this.Cursor == Cursors.VSplit) {
				    if (_resizeHost != null) {
					    ...
					    if (maxWidth < _resizeHost.MinResize) maxWidth = _resizeHost.MinResize;
		    ...
		    private void ColumnHeaderControl_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e) {
			    ...
			    if (e.Button == System.Windows.Forms.MouseButtons.Left) {
				    ...
				    if (this.Cursor == Cursors.VSplit) {
					    ...
					    _resizeStartX = e.X;
					    _resizeCurrentX = e.X;
					    if (_resumePainting) _owner.Invalidate(true);
					    return;
				    }
				    ...
				    foreach (ColumnHost columnHost in _columnHosts) {
					    if (columnHost.Column.Visible) changed = changed || columnHost.mouseDown();
				    }
			    }
		    }
		    private void ColumnHeaderControl_MouseLeave(object sender, System.EventArgs e) {
			    ...
			    foreach (ColumnHost ch in _columnHosts) {
				    if (ch.Column.Visible) changed = changed || ch.mouseLeave();
			    }
			    if (!_onColumnResize) {
			    ...
			    if (e.Button == System.Windows.Forms.MouseButtons.None) {
				    ...
				    if (_owner._checkBoxes) {
					    if (_chkRect.Contains(e.Location)) {
						    if (!_chkHover) {
							    _chkHover = true;
							    if (_chkState == CheckState.Unchecked | _chkState == CheckState.Indeterminate) _currentToolTip = "Check all items.";
							    else _currentToolTip = "Uncheck all items.";
				    ...
				    foreach (ColumnHost ch in _columnHosts) {
					    if (ch.Column.Visible) changed = changed || ch.mouseMove(e);
					    if (ch.OnHover && !ch.OnHoverSplit) {
						    if (!ch.Column.Frozen) _reorderHost = ch;
					    }
				    }
				    ...
				    if (this.Cursor == Cursors.VSplit) {
					    ...
					    if (_resizeHost.Width + dx > _resizeHost.MinResize && _resizeHost.Width + dx < _resizeHost.MaxResize) {
						    ...
						    if (_resizeHost.Width + dx < _resizeHost.MinResize) _resizeCurrentX = _resizeStartX - (_resizeHost.Width - _resizeHost.MinResize);
						    else _resizeCurrentX = _resizeStartX + (_resizeHost.MaxResize - _resizeHost.Width);
					    }
					    changed = changed || (lastX - _resizeCurrentX != 0);
					    if (_resumePainting && changed) _owner.Invalidate(true);
				    } else if (_onColumnReorder) {
					    // Performing column reorder operation.
					    _reorderTarget = null;
					    Rectangle rBound = default(Rectangle);
					    foreach (ColumnHost ch in _columnHosts) {
						    if (ch.Column.Visible) ch.mouseMove(e);
						    rBound = ch.Bounds;
						    rBound.Width = rBound.Width + 2;
						    if (rBound.Contains(e.Location)) _reorderTarget = ch;
					    }
					    if (_reorderTarget == null) {
						    ...
						    if (_reorderTarget == null) {
							    if (e.X < leftHost.X) _reorderTarget = leftHost;
							    if (e.X > rightHost.Right) _reorderTarget = rightHost;
						    }
					    }
					    _reorderCurrent = e.Location;
					    if (_resumePainting) this.Invalidate();
		    ...
		    private void ColumnHeaderControl_MouseUp(object sender, System.Windows.Forms.MouseEventArgs e) {
			    if (_onColumnResize) {
				    ...
				    _onColumnResize = false;
				    if (_resumePainting) _owner.Invalidate(true);
				    if (AfterColumnResize != null) AfterColumnResize(this, new ColumnEventArgs(_resizeHost.Column));
				    _resizeHost = null;
				    return;
			    }
			    if (_onColumnReorder) {
				    ...
				    if (_reorderStart != _reorderCurrent) {
					    if (_reorderHost != _reorderTarget) {
				    ...
				    this.Invalidate();
				    if (colOrderChanged) if (ColumnOrderChanged != null) ColumnOrderChanged(this, new ColumnEventArgs(_reorderHost.Column));
				    _reorderHost = null;
				    _reorderTarget = null;
				    return;
			    }
			    bool changed = false;
			    foreach (ColumnHost ch in _columnHosts) changed = changed || ch.mouseUp();
			    if (changed && _resumePainting) this.Invalidate();
		    }
		    private void ColumnHeaderControl_Paint(object sender, System.Windows.Forms.PaintEventArgs e) {
			    ...
			    foreach (ColumnHost ch in _columnHosts) {
				    if (ch.Column.Visible && !ch.Column.Frozen) {
					    xSeparator = ch.Right;
					    ch.draw(e.Graphics);
					    e.Graphics.DrawLine(Ai.Renderer.Column.NormalBorderPen(), xSeparator, 4, xSeparator, this.Height - 5);
			    ...
			    if (_owner._showColumnOptions) {
				    ...
				    if (_optHover) {
					    if (_optOnDown) bgOpt.InterpolationColors = Ai.Renderer.Column.PressedBlend();
					    else bgOpt.InterpolationColors = Ai.Renderer.Column.HLitedBlend();
					    borderPen = Ai.Renderer.Column.ActiveBorderPen();
				    } else {
					    bgOpt.InterpolationColors = Ai.Renderer.Column.NormalBlend();
				    }
				    ...
				    Ai.Renderer.Drawing.drawTriangle(e.Graphics, _optRect, (this.Enabled ? System.Drawing.Color.FromArgb(21, 66, 139) : System.Drawing.Color.Gray), Color.White, Ai.Renderer.Drawing.TriangleDirection.Down);
				    borderPen.Dispose();
			    }
			    if (_owner._checkBoxes || (_owner._rowNumbers && _owner._view == Control.View.Details)) {
				    int reservedWidth = 0;
				    if (_owner._checkBoxes) reservedWidth += 22;
				    if (_owner._rowNumbers && _owner._view == Control.View.Details) reservedWidth += (int)(e.Graphics.MeasureString(_owner._items.Count.ToString(), _owner.Font).Width + 2);
                    Rectangle chkArea = new Rectangle(0, 0, reservedWidth, this.Height);
                    Pen borderPen = Ai.Renderer.Column.NormalBorderPen();
                    LinearGradientBrush bgChk = new LinearGradientBrush(chkArea, Color.Black, Color.White, LinearGradientMode.Vertical);
                    bgChk.InterpolationColors = Ai.Renderer.Column.NormalBlend();
                    ...
                    if (_owner._checkBoxes)  Ai.Renderer.CheckBox.drawCheckBox(e.Graphics, _chkRect, _chkState, 14, this.Enabled, _chkHover);
				    borderPen.Dispose();
			    }
			    if (_onColumnResize) e.Graphics.DrawLine(Pens.Black, _resizeCurrentX, 0, _resizeCurrentX, this.Height);
			    if (_onColumnReorder) {
			    ...
			    e.Graphics.DrawRectangle(Ai.Renderer.Column.NormalBorderPen(), 0, 0, this.Width - 1, this.Height - 1);
		    }
	    ...
	    private class ListViewItemHost {
		    ...
		    private class ListViewSubItemHost {
			    ...
			    public ListViewSubItemHost(ListViewItemHost owner, ListViewItem.ListViewSubItem subitem) {
				    _owner = owner;
				    _subItem = subitem;
			    }
			    /// <summary>
			    ...
			    public string getSubItemString() {
				    ...
				    if (index == 0) return _owner._item.Text;
			    ...
			    public void measureOriginal() {
				    ...
				    if (aColumn != null) {
					    Font font = null;
					    if (_owner._item.UseItemStyleForSubItems) font = _owner._item.Font;
					    else font = _subItem.Font;
					    _originalSize = _owner._owner._gObj.MeasureString(strSubitem, font);
					    if (_originalSize.Width == 0) _originalSize.Width = 5;
					    if (_originalSize.Height == 0) _originalSize.Height = font.Height;
					    _originalSize.Width += ListView._subItemMargin * 2;
					    _originalSize.Width += 1;
					    _originalSize.Height += ListView._subItemMargin * 2;
					    if (index == 0 && _owner._item.SmallImage != null) _originalSize.Width += 20;
					    if (aColumn.Format == ColumnFormat.Check) _originalSize.Height = 14 + (2 * ListView._subItemMargin);
				    } else {
				    ...
				    if (aColumn != null) {
					    ...
					    if (index == 0) {
						    if (colRect.Width <= Math.Ceiling(_originalSize.Width)) {
							    _displaySize = new Size(colRect.Width, (int)_originalSize.Height);
						    } else {
							    if (aColumn.ColumnAlign != HorizontalAlignment.Left || aColumn.Format == ColumnFormat.Bar || aColumn.Format == ColumnFormat.Check) 
								    _displaySize = new Size(colRect.Width, (int)_originalSize.Height);
							    else 
								    _displaySize = new Size((int)(Math.Ceiling(_originalSize.Width) + 1), (int)_originalSize.Height);
						    }
					    } else {
						    if (colRect.Width <= _originalSize.Width) {
							    ...
							    if (aColumn.ColumnAlign != HorizontalAlignment.Left || aColumn.Format == ColumnFormat.Bar) 
								    ...
								    _displaySize = new Size((int)_originalSize.Width, (int)_originalSize.Height);
						    }
					    }
					    _displaySize.Height += 1;
					    if (!_owner._owner._allowMultiline) {
						    if (strSubitem.IndexOf("\n") > -1) {
			    ...
			    public void draw(Graphics g, bool drawBackground, bool selected) {
				    string strSubitem = getSubItemString();
				    int index = _owner._item.SubItems.IndexOf(_subItem) + 1;
				    ColumnHeader aColumn = _owner._owner._columns[index];
				    if (aColumn == null) return;
				    if (!aColumn.Visible) return;
				    ...
				    if (rect.X > _owner._owner._clientArea.Right || rect.Y > _owner._owner._clientArea.Bottom) return;
				    if (rect.Right < _owner._owner._clientArea.X || rect.Bottom < _owner._owner._clientArea.Y) return;
				    if (drawBackground) {
					    if (_onHover || selected) {
						    ...
						    GraphicsPath hoverPath = Ai.Renderer.Drawing.roundedRectangle(rectBg, 2, 2, 2, 2);
						    Pen borderPen = null;
						    if (selected) {
							    if (_onHover) {
								    borderPen = Ai.Renderer.ListItem.SelectedHLiteBorderPen();
								    hoverBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedHLiteBlend();
							    } else {
								    if (_owner._owner.Focused) {
									    borderPen = Ai.Renderer.ListItem.SelectedBorderPen();
									    hoverBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedBlend();
								    } else {
									    borderPen = Ai.Renderer.ListItem.SelectedBlurBorderPen();
									    hoverBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedBlurBlend();
								    }
							    }
						    } else {
							    borderPen = Ai.Renderer.ListItem.HLitedBorderPen();
							    hoverBrush.InterpolationColors = Ai.Renderer.ListItem.HLitedBlend();
						    }
						    ...
						    if (_owner._item.UseItemStyleForSubItems) g.FillRectangle(new SolidBrush(_owner._item.BackColor), rect);
						    else g.FillRectangle(new SolidBrush(_subItem.BackColor), rect);
					    }
				    }
				    if (aColumn.Format == Control.ColumnFormat.Check) {
					    ...
					    bool checkValue = (bool)_subItem.Value;
					    rectValue.Height = _owner._size.Height;
					    rectCheck.Y = (int)(rectValue.Y + ((rectValue.Height - 14) / 2));
					    switch (aColumn.ColumnAlign) {
						    case HorizontalAlignment.Center:
							    rectCheck.X = (int)(rectValue.X + ((rectValue.Width - 14) / 2));
							    break;
						    case HorizontalAlignment.Left:
					    ...
					    if (checkValue) {
						    Ai.Renderer.CheckBox.drawCheck(g, rectCheck, CheckState.Checked, 14, _owner._owner.Enabled);
					    }
				    } else {
					    ...
					    if (index == 0 && _owner._item.SmallImage != null) {
						    ...
						    rectImg = Ai.Renderer.Drawing.getImageRectangle(_owner._item.SmallImage, rectImg, 16);
						    if (_owner._owner.Enabled) g.DrawImage(_owner._item.SmallImage, rectImg);
						    else Ai.Renderer.Drawing.grayScaledImage(_owner._item.SmallImage, rectImg, g);
						    rectValue.X += 20;
						    rectValue.Width -= 20;
					    }
					    ...
					    if (!_owner._owner._allowMultiline) strFormat.FormatFlags = strFormat.FormatFlags | StringFormatFlags.LineLimit;
					    if (columnFormat == Control.ColumnFormat.Bar) _owner.drawBar(g, new Rectangle(rectValue.X, rectValue.Y + 2, rectValue.Width, rectValue.Height - 4), _subItem, aColumn);
                        if (columnFormat == ColumnFormat.Password) {
                            drawPassword(g, rect, strFormat);
                        } else {
                            if (columnFormat != Control.ColumnFormat.Bar || _subItem.PrintValueOnBar) g.DrawString(strSubitem, font, new SolidBrush(color), rectValue, strFormat);
                        }
					    strFormat.Dispose();
				    }
			    }
                public void draw(Graphics g, bool drawBackground) { draw(g, drawBackground, false); }
			    ...
			    public void draw(Graphics g, Rectangle rect, StringFormat format) {
				    ...
				    Font font = (_owner._item.UseItemStyleForSubItems ? _owner._item.Font : _subItem.Font);
				    ColumnFormat columnFormat = (index == 0 ? Control.ColumnFormat.None : aColumn.Format);
                    if (columnFormat == Control.ColumnFormat.Bar) _owner.drawBar(g, new Rectangle(rect.X, rect.Y, rect.Width - 5, rect.Height), _subItem, aColumn);
                    if (columnFormat == ColumnFormat.Password) {
                        drawPassword(g, rect, format);
                    } else {
                        if (columnFormat != Control.ColumnFormat.Bar || _subItem.PrintValueOnBar) g.DrawString(strSubitem, font, new SolidBrush(color), rect, format);
                    }
			    }
                private void drawPassword(Graphics g, Rectangle rect, StringFormat sf) {
                    int totalWidth = 0;
                    List<Rectangle> rects = new List<Rectangle>();
                    int i = 0;
                    Font font = (_owner._item.UseItemStyleForSubItems ? _owner._item.Font : _subItem.Font);
                    while (totalWidth < rect.Width && i < 5) {
                        rects.Add(new Rectangle(0, 0, (int)(font.Height / 2), (int)(font.Height / 2)));
                        totalWidth += (int)(font.Height / 2) + 3;
                        i++;
                    }
                    int y = rect.Y + (int)((rect.Height - (font.Height / 2)) / 2);
                    int x = rect.X + 3;
                    if (sf.Alignment == StringAlignment.Near) x = rect.X + 1;
                    switch (sf.Alignment) { 
                        ...
                        case StringAlignment.Center:
                            x = rect.X + (int)((rect.Width - totalWidth) / 2);
                            break;
                    }
                    i = 0;
                    while (i < rects.Count) {
                        Rectangle aRect = rects[i];
                        aRect.X = x;
                        aRect.Y = y;
                        x += (int)(font.Height / 2) + 3;
                        g.FillEllipse(Brushes.Black, aRect);
                        i++;
                    }
                }
			    // Mouse event
		    ...
		    public ListViewItemHost(ListViewItem item, ListView owner) {
			    _owner = owner;
			    _item = item;
			    if (_item.Checked) _owner._checkedItems.Add(_item);
			    if (_item.Group != null) _item.Group._checkedItems.Add(_item);
			    refreshSubItem();
		    }
		    // Sub item
		    /// <summary>
		    ...
		    public void relocateSubItems() {
			    foreach (ListViewSubItemHost lvsiHost in _subItemHosts) lvsiHost.moveX();
		    }
		    // Drawing
		    /// <summary>
		    ...
		    private void drawBar(Graphics g, Rectangle rect, ListViewItem.ListViewSubItem subItem, ColumnHeader column) {
			    if (column.MaximumValue > column.MinimumValue) {
				    ...
				    try {
					    double subItemValue = (double)subItem.Value;
					    ...
					    if (valueRange >= 0 & valueRange <= columnRange) {
						    rectValue.Width = (int)Math.Ceiling(valueRange * rectBar.Width / columnRange);
						    ...
						    valueGlowBrush.InterpolationColors = Ai.Renderer.Drawing.BarGlow;
						    if (_owner.Enabled) g.FillRegion(new SolidBrush(subItem.Color), valueRegion);
						    else g.FillRegion(Brushes.Gray, valueRegion);
		    ...
		    public void drawUnFrozen(Graphics g, int frozenCount) {
			    if (!_visible) return;
			    if (_location.X > _owner.Width || _location.Y > _owner.Height) return;
			    if (this.Bottom < 0 || this.Right < 0) return;
			    if (_owner._showGroups && _groupHost.Group.IsCollapsed) return;
			    if (_owner._fullRowSelect) {
				    if (_selected || _onHover) {
					    ...
					    GraphicsPath unfrozenPath = Ai.Renderer.Drawing.roundedRectangle(unfrozenRect, (uint)(frozenCount > 0 ? 0 : 2), 2, (uint)(frozenCount > 0 ? 0 : 2), 2);
					    ...
					    if (_onMouseDown) {
						    unfrozenBrush.InterpolationColors = Ai.Renderer.ListItem.PressedBlend();
						    unfrozenPen = Ai.Renderer.ListItem.PressedBorderPen();
					    } else {
						    if (_selected) {
							    if (_onHover) {
								    unfrozenBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedHLiteBlend();
								    unfrozenPen = Ai.Renderer.ListItem.SelectedHLiteBorderPen();
							    } else {
								    if (_owner.Focused) {
									    unfrozenBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedBlend();
									    unfrozenPen = Ai.Renderer.ListItem.SelectedBorderPen();
								    } else {
									    unfrozenBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedBlurBlend();
									    unfrozenPen = Ai.Renderer.ListItem.SelectedBlurBorderPen();
								    }
							    }
						    } else {
							    unfrozenBrush.InterpolationColors = Ai.Renderer.ListItem.HLitedBlend();
							    unfrozenPen = Ai.Renderer.ListItem.HLitedBorderPen();
						    }
					    }
		    ...
		    public void drawFrozen(Graphics g, int unfrozenCount) {
			    if (!_visible) return;
			    if (_location.X > _owner._clientArea.Right || _location.Y > _owner._clientArea.Bottom) return;
			    if (this.Bottom < 0 || this.Right < 0) return;
			    if (_owner._showGroups && _groupHost.Group.IsCollapsed) return;
			    if (_owner._fullRowSelect) {
				    if (_selected || _onHover) {
					    ...
					    frozenRect.Width -= 1;
					    GraphicsPath frozenPath = Ai.Renderer.Drawing.roundedRectangle(frozenRect, 2, (uint)(unfrozenCount > 0 ? 0 : 2), 2, (uint)(unfrozenCount > 0 ? 0 : 2));
					    LinearGradientBrush frozenBrush = new LinearGradientBrush(frozenRect, Color.Black, Color.White, LinearGradientMode.Vertical);
					    Pen frozenPen = null;
					    if (_onMouseDown) {
						    frozenBrush.InterpolationColors = Ai.Renderer.ListItem.PressedBlend();
						    frozenPen = Ai.Renderer.ListItem.PressedBorderPen();
					    } else {
						    if (_selected) {
							    if (_onHover) {
								    frozenBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedHLiteBlend();
								    frozenPen = Ai.Renderer.ListItem.SelectedHLiteBorderPen();
							    } else {
								    if (_owner.Focused) {
									    frozenBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedBlend();
									    frozenPen = Ai.Renderer.ListItem.SelectedBorderPen();
								    } else {
									    frozenBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedBlurBlend();
									    frozenPen = Ai.Renderer.ListItem.SelectedBlurBorderPen();
								    }
							    }
						    } else {
							    frozenBrush.InterpolationColors = Ai.Renderer.ListItem.HLitedBlend();
							    frozenPen = Ai.Renderer.ListItem.HLitedBorderPen();
						    }
					    }
		    ...
		    public void drawList(Graphics g, bool drawCheckBox) {
			    if (!_visible) return;
			    if (_location.X > _owner._clientArea.Right || _location.Y > _owner._clientArea.Bottom) return;
			    Rectangle itemRect = new Rectangle(_location, _size);
			    if (itemRect.Right < _owner._clientArea.X || itemRect.Bottom < _owner._clientArea.Y) return;
			    ...
			    strFormat.Trimming = StringTrimming.EllipsisCharacter;
			    strFormat.FormatFlags = strFormat.FormatFlags | StringFormatFlags.NoWrap;
			    if (!_owner._allowMultiline) strFormat.FormatFlags = strFormat.FormatFlags | StringFormatFlags.LineLimit;
			    if (_owner._checkBoxes) {
				    textRect.X += 22;
				    textRect.Width -= 22;
			    }
			    imgRect = textRect;
			    imgRect.Width = 20;
			    imgRect = Ai.Renderer.Drawing.getImageRectangle(_item.SmallImage, imgRect, 16);
			    textRect.X += 20;
			    textRect.Width -= 20;
			    if (_selected || _onHover) {
				    GraphicsPath itemPath = Ai.Renderer.Drawing.roundedRectangle(itemRect, 2, 2, 2, 2);
				    LinearGradientBrush itemBrush = new LinearGradientBrush(itemRect, Color.Black, Color.White, LinearGradientMode.Vertical);
				    Pen itemPen = null;
				    if (_onMouseDown) {
					    itemBrush.InterpolationColors = Ai.Renderer.ListItem.PressedBlend();
					    itemPen = Ai.Renderer.ListItem.PressedBorderPen();
				    } else {
					    if (_selected) {
						    if (_onHover) {
							    itemBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedHLiteBlend();
							    itemPen = Ai.Renderer.ListItem.SelectedHLiteBorderPen();
						    } else {
							    if (_owner.Focused) {
								    itemBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedBlend();
								    itemPen = Ai.Renderer.ListItem.SelectedBorderPen();
							    } else {
								    itemBrush.InterpolationColors = Ai.Renderer.ListItem.SelectedBlurBlend();
								    itemPen = Ai.Renderer.ListItem.SelectedBlurBorderPen();
							    }
						    }
					    } else {
						    itemBrush.InterpolationColors = Ai.Renderer.ListItem.HLitedBlend();
						    itemPen = Ai.Renderer.ListItem.HLitedBorderPen();
					    }
				    }
			    ...
			    if ((_onHover || _selected || _item.Checked) && _owner._checkBoxes)
				    Ai.Renderer.CheckBox.drawCheckBox(g, _checkRect, (_item.Checked ? CheckState.Checked : CheckState.Unchecked), 14, _owner.Enabled, _onHoverCheck);
			    if (_item.SmallImage != null) {
				    if (_owner.Enabled) g.DrawImage(_item.SmallImage, imgRect);
				    else Ai.Renderer.Drawing.grayScaledImage(_item.SmallImage, imgRect, g);
			    }
			    g.DrawString(_item.Text, _item.Font, (_owner.Enabled ? new SolidBrush(_item.Color) : Brushes.Gray), textRect, strFormat);
			    strFormat.Dispose();
		    }
            public void drawList(Graphics g) { drawList(g, false); }
		    /// <summary>
		    /// Draw ListViewItem on Tile view mode.
		    /// </summary>
		    [Description("Draw ListViewItem on Tile view mode.")]
		    public void drawTile(Graphics g, bool drawCheckBox) {
			    if (!_visible) return;
			    if (_location.X > _owner._clientArea.Right || _location.Y > _owner._clientArea.Bottom) return;
			    Rectangle itemRect = new Rectangle(_location, _size);
			    if (itemRect.Right < _owner._clientArea.X || itemRect.Bottom < _owner._clientArea.Y) return;
			    ...
			    imgRect = Ai.Renderer.Drawing.getImageRectangle(_item.TileImage, imgRect, ListView._tileHeight - 16);
			    ...
			    if (_item.TileImage != null) {
				    if (_owner.Enabled) g.DrawImage(_item.TileImage, imgRect);
				    else Ai.Renderer.Drawing.grayScaledImage(_item.TileImage, imgRect, g);
			    }
			    int maxIndex = 0;
			    int heightUsed = 0;
			    StringFormat strFormat = new StringFormat();
			    ...
			    while (maxIndex < _subItemHosts.Count && maxIndex < _owner._columns.Count && heightUsed < _size.Height && linesCount < 3) {
				    if (_item.UseItemStyleForSubItems) font = _item.Font;
				    else font = _item.SubItems[maxIndex].Font;
				    if (maxIndex == 0) strSubItem = _item.Text;
				    else strSubItem = _owner.getValueString(_item.SubItems[maxIndex].Value, maxIndex);
				    if (!string.IsNullOrEmpty(strSubItem)) {
					    ...
					    if (heightUsed + strSize.Height > _size.Height) break; // TODO: might not be correct. Was : Exit While
					    heightUsed += (int)strSize.Height;
					    linesCount += itemLines;
				    }
				    maxIndex += 1;
				    if (maxIndex > 0) strFormat.FormatFlags = strFormat.FormatFlags | StringFormatFlags.NoWrap;
			    }
			    maxIndex -= 1;
			    ...
			    while (i <= maxIndex) {
				    ...
				    if (i == 0) strSubItem = _item.Text;
				    else strSubItem = _owner.getValueString(_item.SubItems[i].Value, i);
				    if (!string.IsNullOrEmpty(strSubItem)) {
					    strSize = _owner._gObj.MeasureString(strSubItem, font, new SizeF(textRect.Width, font.Height * (i == 0 ? 2 : 1)), strFormat);
					    _subItemHosts[i].draw(g, new Rectangle(textRect.X, y, textRect.Width, (int)strSize.Height), strFormat);
					    y += (int)strSize.Height;
				    }
				    i += 1;
				    if (i > 0) strFormat.FormatFlags = strFormat.FormatFlags | StringFormatFlags.NoWrap;
			    }
			    strFormat.Dispose();
			    if ((_onHover || _selected || _item.Checked) && _owner._checkBoxes)
				    Ai.Renderer.CheckBox.drawCheckBox(g, _checkRect, (_item.Checked ? CheckState.Checked : CheckState.Unchecked), 14, _owner.Enabled, _onHoverCheck);
		    }
            public void drawTile(Graphics g) { drawTile(g, false); }
		    /// <summary>
		    /// Draw ListViewItem on Icon or Thumbnail view mode.
		    /// </summary>
		    [Description("Draw ListViewItem on Icon or Thumbnail view mode.")]
		    public void drawIconThumbnail(Graphics g, bool drawCheckBox) {
			    if (!_visible) return;
			    if (_location.X > _owner._clientArea.Right || _location.Y > _owner._clientArea.Bottom) return;
			    Rectangle itemRect = new Rectangle(_location, _size);
			    ...
			    if (_selected) {
				    ...
				    if (imgSize + 10 + ListView._subItemMargin + textSize.Height > _size.Height) itemRect.Height = (int)(imgSize + 10 + ListView._subItemMargin + textSize.Height);
			    }
			    if (itemRect.Right < _owner._clientArea.X || itemRect.Bottom < _owner._clientArea.Y) return;
			    ...
			    strFormat.Trimming = StringTrimming.EllipsisCharacter;
			    if (!_owner._allowMultiline) strFormat.FormatFlags = strFormat.FormatFlags | StringFormatFlags.LineLimit;
			    if (_owner._view == Control.View.Icon) img = _item.TileImage;
			    else img = _item.ThumbnailImage;
			    if (img != null) imgRect = Ai.Renderer.Drawing.getImageRectangle(img, imgRect, imgSize);
			    if (_selected || _onHover) {
				    GraphicsPath itemPath = Ai.Renderer.Drawing.roundedRectangle(itemRect, 2, 2, 2, 2);
				    LinearGradientBrush itemBrush = new LinearGradientBrush(itemRect, Color.Black, Color.White, LinearGradientMode.Vertical);
				    Pen itemPen = null;
			    ...
			    if (img != null) {
				    if (_owner.Enabled) g.DrawImage(img, imgRect);
				    else Ai.Renderer.Drawing.grayScaledImage(img, imgRect, g);
			    }
			    g.DrawString(_item.Text, _item.Font, (_owner.Enabled ? new SolidBrush(_item.Color) : Brushes.Gray), textRect, strFormat);
			    if ((_onHover || _selected || _item.Checked) && _owner._checkBoxes)
				    Ai.Renderer.CheckBox.drawCheckBox(g, _checkRect, (_item.Checked ? CheckState.Checked : CheckState.Unchecked), 14, _owner.Enabled, _onHoverCheck);
		    }
            public void drawIconThumbnail(Graphics g) { drawIconThumbnail(g, false); }
		    /// <summary>
		    /// Draw ListViewItem on Preview view mode.
		    /// </summary>
		    [Description("Draw ListViewItem on Preview view mode.")]
		    public void drawPreview(Graphics g, bool drawCheckBox) {
			    if (!_visible) return;
			    if (_location.X > _owner._clientArea.Right || _location.Y > _owner._clientArea.Bottom) return;
			    Rectangle itemRect = new Rectangle(_location, _size);
			    int imgSize = 48;
			    ...
			    strFormat.Trimming = StringTrimming.EllipsisCharacter;
			    if (!_owner._allowMultiline) strFormat.FormatFlags = strFormat.FormatFlags | StringFormatFlags.LineLimit;
			    if (img != null) imgRect = Renderer.Drawing.getImageRectangle(img, imgRect, imgSize);
			    if (_selected | _onHover) Ai.Renderer.Button.draw(g, itemRect, Ai.Renderer.Drawing.ColorTheme.Blue, 2, _owner.Enabled, false, _selected, _onHover, false);
			    if (img != null) {
				    if (_owner.Enabled) g.DrawImage(img, imgRect);
				    else Ai.Renderer.Drawing.grayScaledImage(img, imgRect, g);
			    }
			    g.DrawString(_item.Text, _item.Font, (_owner.Enabled ? (_selected | _onHover ? Ai.Renderer.Column.TextBrush() : Brushes.White) : Brushes.Gray), textRect, strFormat);
			    if ((_onHover || _selected || _item.Checked) && _owner._checkBoxes)
				    Ai.Renderer.CheckBox.drawCheckBox(g, _checkRect, (_item.Checked ? CheckState.Checked : CheckState.Unchecked), 14, _owner.Enabled, _onHoverCheck);
		    }
            public void drawPreview(Graphics g) { drawPreview(g, false); }
		    /// <summary>
		    /// Draw the ListViewItem checkbox on specified Graphics object.
		    /// </summary>
		    public void drawCheckBox(Graphics g) {
			    Ai.Renderer.CheckBox.drawCheckBox(g, _checkRect, (_item.Checked ? System.Windows.Forms.CheckState.Checked : System.Windows.Forms.CheckState.Unchecked), 14, _owner.Enabled, _onHoverCheck);
		    }
		    /// <summary>
		    /// Draw the preview image in the center of the client area.
		    /// </summary>
		    public void drawPreviewImage(Graphics g, Rectangle rect, int degree, bool drawText) {
			    ...
			    Image img = _item.PreviewImage;
			    StringFormat strFormat = new StringFormat();
			    int rounded = (int)(5 * Math.Sin(degree * Math.PI / 180));
			    strFormat.LineAlignment = StringAlignment.Center;
			    strFormat.Alignment = StringAlignment.Center;
			    // Drawing the shadow
			    drawRectangleShadow(new Rectangle(rect.X, rect.Y, rect.Width, rect.Height - 5), Color.Black, g, (int)(5 * Math.Sin(degree * Math.PI / 180)));
			    // Drawing original image
			    // Drawing image's background
			    path = Ai.Renderer.Drawing.roundedRectangle(rect, (uint)rounded, (uint)rounded, (uint)rounded, (uint)rounded);
			    g.FillPath(Brushes.LightGray, path);
			    g.DrawPath(Pens.White, path);
			    // Draw the image, or notification if no image available
			    if (img != null) {
				    imageRect = Ai.Renderer.Drawing.getImageRectangle(img, rect, (int)(rect.Width - (10 * Math.Sin(degree * Math.PI / 180))), (int)(rect.Height - (10 * Math.Sin(degree * Math.PI / 180))));
				    if (_owner.Enabled) g.DrawImage(img, imageRect);
				    else Ai.Renderer.Drawing.grayScaledImage(img, imageRect, g);
			    } else {
				    if (_owner.Enabled) g.DrawString("No Preview Available", _item.Font, Brushes.Black, rect, strFormat);
				    else g.DrawString("No Preview Available", _item.Font, Brushes.Gray, rect, strFormat);
			    }
			    // Darken effect
			    g.FillPath(new SolidBrush(Color.FromArgb((int)(255 - (128 + (127 * Math.Sin(degree * Math.PI / 180)))), 0, 0, 0)), path);
			    g.DrawPath(new Pen(Color.FromArgb((int)(255 - (128 + (127 * Math.Sin(degree * Math.PI / 180)))), 0, 0, 0)), path);
			    path.Dispose();
			    ...
			    drawRectangleShadow(new Rectangle(mirrorRect.X, mirrorRect.Y, mirrorRect.Width, mirrorRect.Height - 5), Color.Black, g, (int)(5 * Math.Sin(degree * Math.PI / 180)));
			    // Drawing image's background
			    path = Ai.Renderer.Drawing.roundedRectangle(mirrorRect, (uint)rounded, (uint)rounded, (uint)rounded, (uint)rounded);
			    g.FillPath(Brushes.Gray, path);
			    g.DrawPath(Pens.White, path);
			    // Draw the image, or notification if no image available
			    if (img != null) {
				    imageRect = Ai.Renderer.Drawing.getImageRectangle(img, mirrorRect, (int)(rect.Width - (10 * Math.Sin(degree * Math.PI / 180))), (int)(rect.Height - (10 * Math.Sin(degree * Math.PI / 180))));
				    if (_owner.Enabled) g.DrawImage(img, imageRect);
				    else Ai.Renderer.Drawing.grayScaledImage(img, imageRect, g);
			    } else {
				    if (_owner.Enabled) g.DrawString("No Preview Available", _item.Font, Brushes.Black, mirrorRect, strFormat);
				    else g.DrawString("No Preview Available", _item.Font, Brushes.Gray, mirrorRect, strFormat);
			    }
			    // Darken effect
			    g.FillPath(new SolidBrush(Color.FromArgb((int)(255 - (128 + (127 * Math.Sin(degree * Math.PI / 180)))), 0, 0, 0)), path);
			    g.DrawPath(new Pen(Color.FromArgb((int)(255 - (128 + (127 * Math.Sin(degree * Math.PI / 180)))), 0, 0, 0)), path);
			    ...
			    if (drawText) {
				    ...
				    txtRect.Height = (int)(txtSize.Height + 4);
				    txtRect.Width = (int)(txtSize.Width + 4);
				    ...
				    GraphicsPath txtPath = Ai.Renderer.Drawing.roundedRectangle(txtRect, 2, 2, 2, 2);
            ...
            public void drawPreviewImage(Graphics g, Rectangle rect, int degree) { drawPreviewImage(g,rect, degree, false); }
		    /// <summary>
		    /// Draw the shadow effect of a rectangle.
		    /// </summary>
		    private void drawRectangleShadow(Rectangle rect, Color color, Graphics g, int rounded, int maxStep) {
			    if (maxStep <= 2) return;
			    int i = 0;
			    GraphicsPath rPath = Ai.Renderer.Drawing.roundedRectangle(rect, (uint)rounded, (uint)rounded, (uint)rounded, (uint)rounded);
			    i = 2;
			    while (i < maxStep) {
            ...
            private void drawRectangleShadow(Rectangle rect, Color color, Graphics g) { drawRectangleShadow(rect, color, g, 3, 20); }
            private void drawRectangleShadow(Rectangle rect, Color color, Graphics g, int rounded) { drawRectangleShadow(rect, color, g, rounded, 20); }
            /// <summary>
            /// Draw ghost image when dragged.
            /// </summary>
            /// <param name="g">Graphics object where the image to be drawn.</param>
            public void drawDrag(Graphics g) {
                Rectangle rc;
                if (_owner._selectedItems.Count == 1) {
                    rc = new Rectangle(0, 0, _dragBmpSize - 5, _dragBmpSize - 5);
                    rc.X = 4;
                    rc.Y = 4;
                    GraphicsPath gpShadow = Renderer.Drawing.roundedRectangle(rc, 3, 3, 3, 3);
                    rc.X = 0;
                    rc.Y = 0;
                    GraphicsPath gpDrag = Renderer.Drawing.roundedRectangle(rc, 3, 3, 3, 3);
                    LinearGradientBrush bgDrag = new LinearGradientBrush(rc, Color.Black, Color.White, LinearGradientMode.Vertical);
                    bgDrag.InterpolationColors = Renderer.Drawing.dragBackgroundBlend();
                    g.FillPath(new SolidBrush(Color.FromArgb(63, 0, 0, 0)), gpShadow);
                    g.FillPath(bgDrag, gpDrag);
                    g.DrawPath(Renderer.Button.NormalBorderPen(), gpDrag);
                    gpShadow.Dispose();
                    gpDrag.Dispose();
                    bgDrag.Dispose();
                } else {
                    rc = new Rectangle(0, 0, _dragBmpSize - 9, _dragBmpSize - 9);
                    rc.X = 8;
                    rc.Y = 8;
                    GraphicsPath gpShadow = Renderer.Drawing.roundedRectangle(rc, 3, 3, 3, 3);
                    g.FillPath(new SolidBrush(Color.FromArgb(63, 0, 0, 0)), gpShadow);
                ...
                switch (_owner._view) { 
                    case View.Details:
                    case View.List:
                        nImg = _item.SmallImage;
                        break;
                    case View.Icon:
                    case View.Tile:
                        nImg = _item.TileImage;
                        break;
                    case View.Thumbnail:
                        nImg = _item.ThumbnailImage;
                        break;
                    default:
                        nImg = _item.PreviewImage;
                        break;
                }
                if (nImg != null) {
                    Rectangle rImg = Renderer.Drawing.getImageRectangle(nImg, rc, (int)(rc.Width - 4));
                    g.DrawImage(nImg, rImg);
                }
                if (_owner._selectedItems.Count > 1) {
                    Font fn = new Font("Segoe UI", 15, FontStyle.Bold);
                    StringFormat sf = new StringFormat();
                    sf.LineAlignment = StringAlignment.Near;
                    sf.Alignment = StringAlignment.Near;
                    rc.X += 4;
                    rc.Y += 4;
                    g.DrawString(_owner._selectedItems.Count.ToString(), fn, Brushes.Black, rc, sf);
                    sf.Dispose();
                    fn.Dispose();
                }
            }
		    ...
		    public void measureSize() {
			    switch (_owner._view) {
				    case Control.View.Details:
					    ...
					    foreach (ListViewSubItemHost siHost in _subItemHosts) {
						    ...
						    if (first) {
							    ...
							    if (siHost.Bounds.X < x) x = siHost.Bounds.X;
							    if (right < siHost.Bounds.Right) right = siHost.Bounds.Right;
							    if (height < siHost.Bounds.Height) height = siHost.Bounds.Height;
						    }
					    }
				    ...
				    case Control.View.List:
					    ...
					    if (_owner._checkBoxes) _size.Width += 22;
					    if (_size.Width > ListView._listColumnWidth - 20) _size.Width = ListView._listColumnWidth - 20;
					    break;
				    case View.Preview:
		    ...
		    public Rectangle getSubItemRectangle(int index) {
			    if (index >= 0 && index < _subItemHosts.Count) return _subItemHosts[index].Bounds;
			    return new Rectangle(0, 0, 0, 0);
		    }
		    /// <summary>
		    ...
		    public string getSubItemString(int index) {
			    if (index >= 0 && index < _subItemHosts.Count) return _subItemHosts[index].getSubItemString();
			    return "";
		    }
		    /// <summary>
		    ...
		    public SizeF getSubItemSize(int index) {
			    if (index >= 0 && index < _subItemHosts.Count) return _subItemHosts[index].OriginalSize;
		    ...
		    public SizeF getTextSize(int maxWidth, int maxHeight) {
			    ...
			    if (_item.SmallImage != null) result.Width += 20;
		    ...
		    public bool needToolTip(Rectangle rect) {
			    ...
			    switch (_owner._view) {
				    case Control.View.Icon:
				    case Control.View.Thumbnail:
					    imgSize = (_owner._view == Control.View.Icon ? _owner._iconSize : _owner._thumbnailSize);
		    ...
		    public int X {
			    ...
			    set {
				    _location.X = value;
				    if (_owner._view == View.Details) _checkRect = new Rectangle(4, _location.Y + ((_size.Height - 14) / 2), 14, 14);
				    else _checkRect = new Rectangle(_location.X + 4, _location.Y + 4, 14, 14);
				    relocateSubItems();
			    }
		    }
		    /// <summary>
		    ...
		    public int Y {
			    ...
			    set {
				    _location.Y = value;
				    foreach (ListViewSubItemHost siHost in _subItemHosts) siHost.Y = _location.Y;
				    if (_owner._view == View.Details) _checkRect = new Rectangle(4, _location.Y + ((_size.Height - 14) / 2), 14, 14);
				    else _checkRect = new Rectangle(_location.X + 4, _location.Y + 4, 14, 14);
			    }
		    }
		    ...
		    public bool Visible {
			    ...
			    set {
				    ...
				    if (!_visible) {
					    _onMouseDown = false;
					    if (_owner._selectedHost == this) _owner._selectedHost = null;
					    _selected = false;
					    _onHover = false;
					    _onHoverCheck = false;
				    }
		    ...
		    public bool IsVisible {
			    get {
				    ...
				    if (_location.X > _owner._clientArea.Right || _location.Y > _owner._clientArea.Bottom) return false;
				    Rectangle rect = Bounds;
				    if (rect.Right < _owner._clientArea.X || rect.Bottom < _owner._clientArea.Y) return false;
				    return true;
			    }
		    }
		    /// <summary>
				    ...
				    if (_selected) {
					    if (_owner._view == Control.View.Icon || _owner._view == Control.View.Thumbnail) {
						    ...
						    if (imgSize + 10 + ListView._subItemMargin + textSize.Height > _size.Height)
							    rectArea.Height = (int)(imgSize + 10 + ListView._subItemMargin + textSize.Height);
					    }
				    }
				    return rectArea;
			    }
		    ...
		    public int RowIndex {
			    get {
				    ...
				    foreach (ListViewGroupHost lvgHost in _owner._groupHosts) {
					    if (lvgHost != _groupHost) index += lvgHost.VisibleItemsCount;
					    else break; // TODO: might not be correct. Was : Exit For
				    }
				    foreach (ListViewItemHost lviHost in _groupHost.ItemHosts) {
					    if (!object.ReferenceEquals(lviHost, this)) {
						    if (lviHost.Visible && !lviHost.Frozen) index += 1;
					    } else {
						    break; // TODO: might not be correct. Was : Exit For
					    }
				    }
			    ...
			    if (_owner._view == Control.View.Details) {
				    ...
				    if (_owner._fullRowSelect) {
					    ...
					    if (itemRect.Contains(e.Location) && e.X > _owner._clientArea.X && e.Y > _owner._clientArea.Y) {
						    if (!_onHover) {
							    _owner.invokeItemHover(this);
							    _onHover = true;
							    if (_owner._itemTooltip) {
								    if (Ai.Renderer.ToolTip.containsToolTip(_item.ToolTipTitle, _item.ToolTip, _item.ToolTipImage)) {
								    ...
								    if (columns.Count == 0) columns = _owner._columnControl.UnFrozenColumns;
								    if (columns.Count > 0) {
						    ...
						    if (_onHover) {
							    _onHover = false;
							    if (_owner._tooltipCaller == this) _owner._tooltip.hide();
							    stateChanged = true;
						    }
					    }
					    ...
					    if (columns.Count > 0) {
						    ...
						    if (rectFrozen.Contains(e.Location)) {
							    if (subitemHover != null) subitemHover.OnHover = false;
							    subitemHover = null;
							    hoverIndex = -1;
						    }
					    }
				    ...
				    if (itemRect.Contains(e.Location) && e.X > _owner._clientArea.X && e.Y > _owner._clientArea.Y) {
					    ...
					    if (_owner._selectedHost != this) {
						    if (_owner._selectedHost != null) {
							    if (_owner._selectedHost.IsVisible) {
								    if (_owner._selectedHost.Bounds.Contains(e.Location)) mouseOnArea = false;
							    }
						    }
					    }
					    if (mouseOnArea) {
				    ...
				    if (!_onHoverCheck) {
					    if (_owner._selectedHost != this) {
						    ...
						    if (button == System.Windows.Forms.MouseButtons.Left) {
							    if (_owner._labelEdit) _owner.showTextBoxEditor(this);
						    }
					    }
					    if (!_onMouseDown) {
						    _onMouseDown = true;
            ...
            public bool mouseDown() { return mouseDown(System.Windows.Forms.MouseButtons.Left); }
		    /// <summary>
		    ...
		    public bool mouseLeave() {
			    bool stateChanged = false;
			    if (_owner._view == Control.View.Details) {
				    foreach (ListViewSubItemHost lvsiHost in _subItemHosts) stateChanged = stateChanged || lvsiHost.mouseLeave();
			    }
            ...
            /// <summary>
            /// Selects or deselects ListViewItemHost object determined by contained listviewitem.
            /// </summary>
            /// <param name="item">ListViewItem object to determine where the host must be selected or deselected.</param>
            /// <param name="selected">Determine whether the host is selected or deselected.</param>
            /// <returns>True if ListViewItem object is found within the host, false otherwise.</returns>
            public bool selectHost(ListViewItem item, bool selected) {
                if (_item == item) {
                    _selected = selected;
                    if (_selected) _owner._selectedHost = this;
                    return true;
                }
                return false;
            }
            public bool selectHost(bool selected) {
	    ...
	    private class ListViewGroupHost {
					    ...
					    foreach (ListViewItemHost lviHost in _itemHosts) {
						    if (lviHost.Visible) {
							    ...
							    if (itemsX + xRange > maxWidth) {
								    if (_owner._view == View.List) yRange += 1;
								    itemsY = itemsY + yRange;
								    _groupSize.Width = itemsX;
								    itemsX = _location.X;
								    if (_owner._view == View.List) yRange = 0;
							    }
						    }
					    }
					    break;
				    case Control.View.Preview:
					    ...
					    if (_owner._slideLocation == Control.SlideLocation.Top | _owner._slideLocation == Control.SlideLocation.Bottom) {
						    ...
						    foreach (ListViewItemHost lviHost in _itemHosts) {
							    lviHost.X = itemsX;
							    lviHost.Y = itemsY;
							    if (lviHost.Visible) itemsX += lviHost.Bounds.Width + 5;
						    }
					    } else {
						    itemsY = _location.Y + _headerSize.Height + 1;
						    foreach (ListViewItemHost lviHost in _itemHosts) {
							    lviHost.Y = itemsY;
							    lviHost.X = itemsX;
							    if (lviHost.Visible) itemsY += lviHost.Bounds.Height + 5;
						    }
					    }
					    break;
			    }
		    ...
		    /// </summary>
		    public void measureSize() {
			    foreach (ListViewItemHost lviHost in _itemHosts) lviHost.measureSize();
			    ...
			    switch (_owner._view) {
				    case Control.View.Details:
					    ...
					    Rectangle frozenRect = _owner._columnControl.FrozenRectangle;
					    if (frozenRect.X > -1) _location.X = frozenRect.X;
					    _groupSize.Width = _owner._columnControl.DisplayedRectangle.Width;
					    ...
					    foreach (ListViewItemHost lviHost in _itemHosts) {
						    if (lviHost.Visible & !lviHost.Frozen) itemsHeight += lviHost.Bounds.Height + 1;
					    }
					    ...
					    if (_groupSize.Height > 0) {
						    ...
						    if (_headerSize.Height < 22) _headerSize.Height = 22;
				    ...
				    case Control.View.List:
					    ...
					    foreach (ListViewItemHost lviHost in _itemHosts) {
						    if (lviHost.Visible) {
							    ...
							    if (_owner._view == View.List) {
								    if (yRange < lviHost.Bounds.Height) yRange = lviHost.Bounds.Height;
							    }
					    ...
					    if (itemsWidth > 0) itemsHeight += yRange;
					    if (_groupSize.Width < maxWidth) _groupSize.Width = maxWidth;
					    if (itemsHeight > 0) _groupSize.Height = itemsHeight;
					    _headerSize.Width = _groupSize.Width;
					    if (_groupSize.Height > 0) {
						    _headerSize.Height = _owner._groupFont.Height;
						    if (_headerSize.Height < 22) _headerSize.Height = 22;
				    ...
				    case Control.View.Preview:
					    ...
					    if (_owner._slideLocation == Control.SlideLocation.Top || _owner._slideLocation == Control.SlideLocation.Bottom) {
						    foreach (ListViewItemHost lviHost in _itemHosts) {
							    if (lviHost.Visible) itemsWidth += lviHost.Bounds.Width + 5;
						    }
						    if (itemsWidth > 0) {
							    _groupSize.Height = _headerSize.Height;
						    ...
						    foreach (ListViewItemHost lviHost in _itemHosts) {
							    if (lviHost.Visible) itemsHeight += lviHost.Bounds.Height + 5;
						    }
						    if (itemsHeight > 0) {
		    ...
		    public void sortItems() { _itemHosts.Sort(_owner.itemHostComparer); }
		    /// <summary>
		    ...
		    public void filterItems() {
			    ...
			    foreach (ListViewItemHost ih in _itemHosts) ih.Visible = _owner.filterItem(ih.Item, handlers);
		    }
		    /// <summary>
		    ...
		    public void removeHost(ListViewItem item) {
			    ...
			    if (index > -1) _itemHosts.RemoveAt(index);
		    }
		    // Check Box
		    /// <summary>
		    ...
		    public void checkCheckedState() {
			    int checkedCount = 0;
			    foreach (ListViewItemHost lviHost in _itemHosts) {
				    if (lviHost.Item.Checked) checkedCount += 1;
			    }
			    if (checkedCount == _itemHosts.Count) {
				    _group._checkState = CheckState.Checked;
			    } else {
				    if (checkedCount == 0) _group._checkState = CheckState.Unchecked;
				    else _group._checkState = CheckState.Indeterminate;
			    }
		    }
		    // Public properties
		    /// <summary>
		    ...
		    public Size GroupSize {
			    get {
				    if (_group.IsCollapsed) return _headerSize;
				    else return _groupSize;
			    }
		    }
		    /// <summary>
		    ...
		    public List<ListViewItemHost> VisibleItemHosts {
			    get {
				    ...
				    foreach (ListViewItemHost lviHost in _itemHosts) {
					    if (lviHost.Visible && !lviHost.Frozen) result.Add(lviHost);
				    }
				    return result;
			    }
		    }
		    ...
		    public List<ListViewItemHost> ItemHosts { get { return _itemHosts; } }
		    /// <summary>
		    ...
		    public Rectangle Bounds {
			    get {
				    if (_groupSize.Width == 0 || _groupSize.Height == 0) {
					    ...
					    if (_group.IsCollapsed) return new Rectangle(_location, _headerSize);
					    else return new Rectangle(_location, _groupSize);
				    }
			    }
		    }
		    /// <summary>
		    ...
		    public int Right {
			    get {
				    if (_groupSize.Width == 0) {
					    ...
					    if (_group.IsCollapsed) return _location.X + _headerSize.Width;
					    else return _location.X + _groupSize.Width;
				    }
			    }
		    }
		    /// <summary>
		    ...
		    public int Bottom {
			    get {
				    if (_groupSize.Height == 0) {
					    ...
					    if (_group.IsCollapsed) return _location.Y + _headerSize.Height;
					    else return _location.Y + _groupSize.Height;
				    }
			    }
		    }
		    /// <summary>
		    ...
		    public bool ItemIsVisible(ListViewItem item) {
		        foreach (ListViewItemHost ih in _itemHosts) {
				    if (ih.Item == item) return ih.Visible;
			    }
			    return false;
		    }
		    /// <summary>
		    ...
		    public int VisibleItemsCount {
			    get {
				    int result = 0;
				    foreach (ListViewItemHost lviHost in _itemHosts) {
					    if (lviHost.Visible && !lviHost.Frozen) result += 1;
				    }
				    return result;
			    }
		    }
		    ...
		    public void draw(Graphics g, bool drawCheckBox) {
			    if (VisibleItemsCount == 0) return;
			    if (!_owner._showGroups) return;
			    if (_groupSize.Width == 0 || _groupSize.Height == 0) return;
			    if (_location.X > _owner._clientArea.Right || _location.Y > _owner._clientArea.Bottom) return;
			    if (this.Right < 0 || this.Bottom < 0) return;
			    ...
			    foreach (ListViewItemHost ih in _itemHosts) {
				    if (ih.Visible && !ih.Frozen) visibleCount = visibleCount + 1;
			    }
			    if (_owner._view == View.Preview && (_owner._slideLocation == SlideLocation.Bottom || _owner._slideLocation == SlideLocation.Top)) {
				    ...
				    if (_owner._checkBoxes) txtRect.Y = _chkRect.Bottom + 2;
				    txtRect.Height = _headerSize.Height - ((txtRect.Y - _location.Y) + 2);
				    txtRect.Width = _headerSize.Width;
				    // Draw drop down
				    if (_owner.Enabled) Ai.Renderer.Drawing.drawTriangle(g, _ddRect, (_ddHover ? System.Drawing.Color.Gold : System.Drawing.Color.White), Color.Black, (_group.IsCollapsed ? Renderer.Drawing.TriangleDirection.Down : Renderer.Drawing.TriangleDirection.DownRight));
				    else Ai.Renderer.Drawing.drawTriangle(g, _ddRect, Color.Gray, Color.Black, (_group.IsCollapsed ? Renderer.Drawing.TriangleDirection.Down : Renderer.Drawing.TriangleDirection.DownRight));
				    // Draw checkbox
				    if (_owner._checkBoxes && drawCheckBox) Ai.Renderer.CheckBox.drawCheckBox(g, _chkRect, _group._checkState, 14, _owner.Enabled, _chkHover);
				    ...
				    if (_owner._checkBoxes) txtRect.X = _chkRect.Right + 2;
				    txtRect.Width = _headerSize.Width - ((txtRect.X - _location.X) + 2);
				    txtRect.Height = _headerSize.Height;
				    // Draw drop down
				    if (_owner.Enabled) {
					    if (_owner._view == Control.View.Preview) Ai.Renderer.Drawing.drawTriangle(g, _ddRect, (_ddHover ? System.Drawing.Color.Gold : System.Drawing.Color.White), Color.Black, (_group.IsCollapsed ? Renderer.Drawing.TriangleDirection.Right : Renderer.Drawing.TriangleDirection.DownRight));
					    else Ai.Renderer.Drawing.drawTriangle(g, _ddRect, (_ddHover ? System.Drawing.Color.Gold : System.Drawing.Color.Black), Color.White, (_group.IsCollapsed ? Renderer.Drawing.TriangleDirection.Right : Renderer.Drawing.TriangleDirection.DownRight));
				    } else {
					    Ai.Renderer.Drawing.drawTriangle(g, _ddRect, Color.Gray, (_owner._view == Control.View.Preview ? Color.Black : Color.White), (_group.IsCollapsed ? Renderer.Drawing.TriangleDirection.Right : Renderer.Drawing.TriangleDirection.DownRight));
				    }
				    // Draw checkbox
				    if (_owner._checkBoxes && (drawCheckBox || _group._checkState != CheckState.Unchecked)) Ai.Renderer.CheckBox.drawCheckBox(g, _chkRect, _group._checkState, 14, _owner.Enabled, _chkHover);
				    // Draw header and underline
		    ...
		    public void drawItemsCheckBox(Graphics g) {
			    ...
			    if (_location.X > _owner._clientArea.Right || _location.Y > _owner._clientArea.Bottom) return;
			    if (this.Right < 0 || this.Bottom < 0) return;
			    if (_group.IsCollapsed) return;
			    foreach (ListViewItemHost lviHost in _itemHosts) {
				    if (lviHost.IsVisible) lviHost.drawCheckBox(g);
			    }
		    }
		    /// <summary>
		    ...
		    public void drawItemsRowNumber(Graphics g, StringFormat rowFormat, int x, int width, Brush rowBrush) {
			    ...
			    foreach (ListViewItemHost lviHost in _itemHosts) {
				    if (lviHost.IsVisible && !lviHost.Frozen) {
					    ...
					    g.DrawString(lviHost.RowIndex.ToString(), _owner.Font, rowBrush, rectRowNumber, rowFormat);
				    }
			    }
		    }
		    // Mouse event handler
		    ...
		    /// </summary>
		    public bool mouseMove(MouseEventArgs e) {
			    if (!_owner._showGroups) return false;
			    if (_groupSize.Width == 0 || _groupSize.Height == 0) return false;
			    if (_location.X > _owner.Width || _location.Y > _owner.Height) return false;
			    if (this.Bottom < 0 || this.Right < 0) return false;
			    bool stateChanged = false;
			    ...
			    if (!_group.IsCollapsed) {
				    foreach (ListViewItemHost ih in _itemHosts) {
					    if (ih.Visible) stateChanged = stateChanged || ih.mouseMove(e);
				    }
			    }
			    return stateChanged;
		    }
		    ...
		    public bool mouseDown(MouseButtons button) {
			    ...
			    if (_ddHover || _chkHover) {
				    if (_ddHover) {
					    ...
					    if (_group.IsCollapsed) _group.expand();
					    else _group.collapse();
					    ...
					    if (_group._checkState == CheckState.Checked) {
						    ...
						    if (!_group.cancelStatus()) {
							    foreach (ListViewItemHost lviHost in _itemHosts) lviHost.Item.setChecked(false);
			    ...
			    if (!_group.IsCollapsed) {
				    foreach (ListViewItemHost ih in _itemHosts) {
					    if (ih.Visible) stateChanged = stateChanged || ih.mouseDown(button);
				    }
			    }
			    return stateChanged;
		    }
		    ...
		    public bool mouseLeave() {
			    ...
			    if (!_group.IsCollapsed) {
				    foreach (ListViewItemHost ih in _itemHosts) {
					    if (ih.Visible) stateChanged = stateChanged || ih.mouseLeave();
				    }
			    }
			    return stateChanged;
		    }
		    ...
		    public bool mouseUp(MouseEventArgs e) {
			    ...
			    if (!_group.IsCollapsed) {
				    foreach (ListViewItemHost ih in _itemHosts) {
					    if (ih.Visible) stateChanged = stateChanged || ih.mouseUp(e);
				    }
			    }
			    return stateChanged;
		    }
	    ...
	    private class ItemSlider {
		    ...
		    public ItemSlider(ListView owner) { _owner = owner; }
		    /// <summary>
			    ...
			    set {
				    if (_orientation != value) {
					    ...
					    if (_orientation == System.Windows.Forms.Orientation.Horizontal) {
						    _height = 5;
						    _slideRect.X = _location.X + (int)(_width * _value / _max) - 3;
						    _slideRect.Y = _location.Y;
					    } else {
						    _width = 5;
						    _slideRect.X = _location.X;
						    _slideRect.Y = _location.Y + (int)(_height * _value / _max) - 3;
					    }
				    }
			    }
		    }
			    ...
			    set {
				    if (_value != value) {
					    if (value >= 0 && value <= _max) {
						    _value = value;
						    if (_orientation == System.Windows.Forms.Orientation.Horizontal) _slideRect.X = _location.X + (int)(_width * _value / _max) - 3;
						    else _slideRect.Y = _location.Y + (int)(_height * _value / _max) - 3;
						    _changed = true;
		    ...
		    public int Width {
			    get { return _width; }
			    set {
				    if (_orientation == System.Windows.Forms.Orientation.Vertical) return;
		    ...
		    public int Height {
			    get { return _height; }
			    set {
				    if (_orientation == System.Windows.Forms.Orientation.Horizontal) return;
		    ...
		    /// </summary>
		    public int Right { get { return _location.X + _width; } }
		    /// <summary>
		    ...
		    public void draw(Graphics g) {
			    if (!_visible) return;
			    if (_orientation == System.Windows.Forms.Orientation.Horizontal) {
				    if (_owner.Enabled) g.DrawLine(Pens.White, _location.X, _location.Y + 2, _location.X + _width, _location.Y + 2);
				    else g.DrawLine(Pens.Gray, _location.X, _location.Y + 2, _location.X + _width, _location.Y + 2);
			    } else {
				    if (_owner.Enabled) g.DrawLine(Pens.White, _location.X + 2, _location.Y, _location.X + 2, _location.Y + _height);
				    else g.DrawLine(Pens.Gray, _location.X + 2, _location.Y, _location.X + 2, _location.Y + _height);
			    }
			    drawSlider(g);
			    _changed = false;
		    }
			    ...
			    if (_visible) {
				    if (!_interceptMouseDown) {
					    if (_slideRect.Contains(p)) {
						    ...
						    if (rectArea.Contains(p)) _boxPoint = new Point(p.X - _location.X, p.Y - _location.Y);
					    ...
					    float _newValue = 0;
					    if (_orientation == System.Windows.Forms.Orientation.Horizontal) _newValue = (p.X - _location.X) * _max / _width;
					    else _newValue = (p.Y - _location.Y) * _max / _height;
			    ...
			    if (_visible) {
				    if (_hoverSlide) {
					    ...
					    if (_boxPoint.X > 0 || _boxPoint.Y > 0) {
						    float _newValue = 0;
						    if (_orientation == System.Windows.Forms.Orientation.Horizontal) _newValue = _boxPoint.X * _max / _width;
						    else _newValue = _boxPoint.Y * _max / _height;
						    if (_newValue < 0) _newValue = 0;
						    if (_newValue > _max) _newValue = _max;
						    Value = _newValue;
					    }
	    ...
	    private int getSubItemMaxWidth(int index) {
		    ...
		    foreach (ListViewItemHost ih in _itemHosts) {
			    ...
			    if (maxWidth < iSize.Width) maxWidth = (int)Math.Ceiling(iSize.Width);
		    }
		    return maxWidth;
	    }
	    /// <summary>
	    ...
	    private int getSubItemMaxWidth(ColumnHeader column) { return getSubItemMaxWidth(_columns.IndexOf(column)); }
	    /// <summary>
	    ...
	    private int itemHostComparer(ListViewItemHost host1, ListViewItemHost host2) {
		    ...
		    if (host1.Visible && host2.Visible) {
			    ...
			    while (i < _columns.Count) {
				    if (i != _columnRef) colsToCompare.Add(i);
				    i = i + 1;
			    }
			    i = 0;
			    while (i < colsToCompare.Count && result == 0) {
				    ...
				    try {
					    ...
					    if (_columns[currentColumn].SortOrder == SortOrder.Ascending) result = objCompare.Compare(vNode1, vNode2);
					    else result = -objCompare.Compare(vNode1, vNode2);
				    } catch (Exception) {
					    result = 0;
				    }
				    i++;
			    ...
			    if (host1.Visible) {
				    if (_columns[_columnRef].SortOrder == SortOrder.Ascending) return 1;
				    else return -1;
	    ...
	    private int groupHostComparer(ListViewGroupHost host1, ListViewGroupHost host2) {
		    if (_columnRef == -1) return 0;
		    if (_columns[_columnRef].SortOrder == SortOrder.Ascending) return string.Compare(host1.Group.Text, host2.Group.Text, true);
		    else return -string.Compare(host1.Group.Text, host2.Group.Text, true);
	    }
	    /// <summary>
	    ...
	    private void changeColumnCheckState() {
		    ...
		    if (checkedCount == _itemHosts.Count) {
			    ...
			    if (checkedCount == 0) _columnControl.CheckState = CheckState.Unchecked;
			    else _columnControl.CheckState = CheckState.Indeterminate;
		    }
	    }
	    /// <summary>
	    ...
	    private void sortAll() {
		    if (_columnRef == -1) return;
		    if (_showGroups) {
			    _groupHosts.Sort(groupHostComparer);
			    foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.sortItems();
	    ...
	    private void measureAll() {
            try {
                ...
                if (_itemHosts.Count == 0) {
                    if (_view == Control.View.Details) {
                        ...
                        if (_checkBoxes || _rowNumbers) {
                            ...
                            if (_rowNumbers) {
                                _clientArea.X += (int)_gObj.MeasureString("0", Font).Width + 2;
                                _clientArea.Width -= (int)(_gObj.MeasureString("0", Font).Width + 2);
                            }
                        }
                        _hScroll.Left = 1;
                ...
                int itemsHeight = 0;
                int itemsWidth = 0;
                if (_view == Control.View.Details) itemsWidth = _columnControl.ColumnsWidth;
                if (_showGroups) {
                    if (_view == Control.View.Preview && (_slideLocation == Control.SlideLocation.Bottom || _slideLocation == Control.SlideLocation.Top)) {
                        foreach (ListViewGroupHost lvgHost in _groupHosts) {
                            lvgHost.measureSize();
                            if (lvgHost.IsVisible) itemsWidth += lvgHost.GroupSize.Width + 5;
                        }
                        _defaultGroupHost.measureSize();
                        if (_defaultGroupHost.IsVisible) itemsWidth += _defaultGroupHost.GroupSize.Width;
                        itemsHeight = _slideItemSize + 5;
                    } else {
                        foreach (ListViewGroupHost lvgHost in _groupHosts) {
                            lvgHost.measureSize();
                            if (lvgHost.IsVisible) {
                                if (itemsWidth < lvgHost.GroupSize.Width) itemsWidth = lvgHost.GroupSize.Width;
                                itemsHeight += lvgHost.GroupSize.Height + 5;
                            }
                        }
                        _defaultGroupHost.measureSize();
                        if (_defaultGroupHost.IsVisible) {
                            if (itemsWidth < _defaultGroupHost.GroupSize.Width) itemsWidth = _defaultGroupHost.GroupSize.Width;
                            itemsHeight += _defaultGroupHost.GroupSize.Height;
                        }
                        if (_view == Control.View.Preview) itemsWidth = _slideItemSize + 5;
                    }
                    // Calculating the frozen items if _view = Details
                    if (_view == Control.View.Details) {
                        foreach (ListViewItemHost lviHost in _frozenHosts) {
                            lviHost.measureSize();
                            if (lviHost.Visible) itemsHeight += lviHost.Bounds.Height + 1;
                        }
                    }
                } else {
                    foreach (ListViewItemHost lviHost in _itemHosts) lviHost.measureSize();
                        ...
                        case Control.View.List:
                            ...
                            foreach (ListViewItemHost ih in _itemHosts) {
                                if (ih.Visible) {
                                    ...
                                    if (_view == View.List) {
                                        if (yRange < ih.Bounds.Height) yRange = ih.Bounds.Height;
                                    }
                                    if (rowWidth + xRange > maxWidth) {
                                        if (_view == View.List) yRange += 1;
                                        itemsHeight = itemsHeight + yRange;
                                        itemsWidth = rowWidth;
                                        rowWidth = 0;
                                        if (_view == View.List)
                            ...
                            if (rowWidth > 0) itemsHeight += yRange;
                            break;
                        case Control.View.Preview:
                ...
                switch (_view) {
                    case Control.View.Details:
                        if (_checkBoxes) _clientArea.X += 22;
                        if (_rowNumbers) _clientArea.X += (int)_gObj.MeasureString(Convert.ToString(_items.Count), Font).Width + 2;
                        ...
                        foreach (ListViewItemHost lviHost in _frozenHosts) {
                            if (lviHost.Visible) _frozenArea.Height += lviHost.Bounds.Height + 1;
                        }
                        if (_frozenRowsDock == RowsDock.Top) _frozenArea.Y = _clientArea.Y - 1;
                        else _frozenArea.Y = _clientArea.Bottom - _frozenArea.Height;
                        break;
                    case Control.View.Preview:
                        _vScroll.Visible = false;
                        _hScroll.Visible = false;
                        ...
                        if (_slideLocation == Control.SlideLocation.Bottom || _slideLocation == Control.SlideLocation.Top) {
                            ...
                            if (_slideLocation == Control.SlideLocation.Bottom) _slider.Location = new Point(10, this.Height - (itemsHeight + _slider.Height + 1));
                            else _slider.Location = new Point(10, _clientArea.Y + itemsHeight);
                            _slider.Width = _clientArea.Width - 20;
                            _slider.Visible = itemsWidth > _clientArea.Width;
                            if (_slider.Visible) _slider.Maximum = itemsWidth - _clientArea.Width;
                        } else {
                            _slider.Orientation = Orientation.Vertical;
                            if (_slideLocation == Control.SlideLocation.Left) _slider.Location = new Point(itemsWidth + 1, _clientArea.Y + 10);
                            else _slider.Location = new Point(this.Width - (itemsWidth + _slider.Width + 1), _clientArea.Y + 10);
                            _slider.Height = _clientArea.Height - 20;
                            _slider.Visible = itemsHeight > _clientArea.Height;
                            if (_slider.Visible) _slider.Maximum = itemsHeight - _clientArea.Height;
                        }
                        break;
                    default:
                        _clientArea.Width = this.Width - (_vScroll.Width + _clientArea.X);
	    ...
	    private void relocateAll() {
		    ...
		    if (_view == Control.View.Preview) {
			    if (_slider.Visible) {
				    if (_slideLocation == Control.SlideLocation.Bottom || _slideLocation == Control.SlideLocation.Top) x -= (int)_slider.Value;
				    else y -= (int)_slider.Value;
			    }
		    } else {
			    if (_view != Control.View.Details) {
				    if (_hScroll.Visible) x -= _hScroll.Value;
			    }
			    if (_vScroll.Visible) y -= _vScroll.Value;
		    }
		    if (_view == Control.View.Details) {
			    // Relocate all frozen items
			    if (_frozenRowsDock == RowsDock.Top) _frozenArea.Y = _clientArea.Y - 2;
			    else _frozenArea.Y = _clientArea.Bottom - _frozenArea.Height;
			    int frozenY = _frozenArea.Y;
			    foreach (ListViewItemHost lviHost in _frozenHosts) {
				    lviHost.Y = frozenY;
				    if (lviHost.Visible) frozenY = lviHost.Bottom + 1;
			    }
		    }
		    if (_showGroups) {
			    switch (_view) {
				    case Control.View.Details:
					    if (_frozenRowsDock == RowsDock.Top) y += _frozenArea.Height;
					    foreach (ListViewGroupHost lvgHost in _groupHosts) {
						    lvgHost.Y = y;
						    lvgHost.relocate();
						    if (lvgHost.IsVisible) y = lvgHost.Bottom + 5;
					    }
				    ...
				    case Control.View.Preview:
					    if (_slideLocation == Control.SlideLocation.Bottom | _slideLocation == Control.SlideLocation.Top) {
						    foreach (ListViewGroupHost lvgHost in _groupHosts) {
							    if (_slideLocation == Control.SlideLocation.Top) lvgHost.Y = _clientArea.Y;
							    else lvgHost.Y = _clientArea.Bottom - (_slideItemSize + 1);
							    lvgHost.X = x;
							    lvgHost.relocate();
							    if (lvgHost.IsVisible) x = lvgHost.Right + 5;
						    }
						    if (_slideLocation == Control.SlideLocation.Top) _defaultGroupHost.Y = _clientArea.Y;
						    else _defaultGroupHost.Y = _clientArea.Bottom - (_slideItemSize + 1);
						    _defaultGroupHost.X = x;
						    _defaultGroupHost.relocate();
					    } else {
						    foreach (ListViewGroupHost lvgHost in _groupHosts) {
							    if (_slideLocation == Control.SlideLocation.Left) lvgHost.X = _clientArea.X;
							    else lvgHost.X = _clientArea.Right - (_slideItemSize + 1);
							    lvgHost.Y = y;
							    lvgHost.relocate();
							    if (lvgHost.IsVisible) y = lvgHost.Bottom + 5;
						    }
						    if (_slideLocation == Control.SlideLocation.Left) _defaultGroupHost.X = _clientArea.X;
						    else _defaultGroupHost.X = _clientArea.Right - (_slideItemSize + 1);
						    _defaultGroupHost.Y = y;
						    _defaultGroupHost.relocate();
					    }
					    break;
			    ...
			    switch (_view) {
				    case Control.View.Details:
					    ...
					    foreach (ListViewItemHost lviHost in _itemHosts) {
						    lviHost.Y = y;
						    lviHost.relocateSubItems();
						    if (lviHost.Visible && !lviHost.Frozen) y = lviHost.Bounds.Bottom + 1;
					    }
					    break;
				    case Control.View.Preview:
					    if (_slideLocation == Control.SlideLocation.Bottom || _slideLocation == Control.SlideLocation.Top) {
						    foreach (ListViewItemHost lviHost in _itemHosts) {
							    if (_slideLocation == Control.SlideLocation.Top) lviHost.Y = _clientArea.Y;
							    else lviHost.Y = _clientArea.Bottom - (_slideItemSize + 1);
							    lviHost.X = x;
							    if (lviHost.Visible) x = lviHost.Right + 5;
						    }
					    } else {
						    foreach (ListViewItemHost lviHost in _itemHosts) {
							    if (_slideLocation == Control.SlideLocation.Left) lviHost.X = _clientArea.X;
							    else lviHost.X = _clientArea.Right - (_slideItemSize + 1);
							    lviHost.Y = y;
							    if (lviHost.Visible) y = lviHost.Bottom + 5;
						    }
					    }
					    break;
				    default:
	    ...
	    /// </summary>
        private ListViewItemHost getUpperHost(ListViewItemHost aHost, List<ListViewItemHost> itemHosts) { return getUpperHost(aHost, itemHosts, false); }
	    private ListViewItemHost getUpperHost(ListViewItemHost aHost, List<ListViewItemHost> itemHosts, bool searchFromBottom) {
		    if (aHost == null) return null;
		    if (_itemHosts.Count == 0) return null;
		    int hostIndex = itemHosts.IndexOf(aHost) - 1;
		    if (hostIndex < 0) {
			    if (searchFromBottom) hostIndex = itemHosts.Count - 1;
		    }
		    while (hostIndex >= 0) {
			    if (itemHosts[hostIndex].Visible) {
				    if (itemHosts[hostIndex].X == aHost.X) return itemHosts[hostIndex];
			    }
			    hostIndex -= 1;
		    }
		    return null;
	    ...
	    /// </summary>
        private ListViewItemHost getLowerHost(ListViewItemHost aHost, List<ListViewItemHost> itemHosts) { return getLowerHost(aHost, itemHosts, false); }
	    private ListViewItemHost getLowerHost(ListViewItemHost aHost, List<ListViewItemHost> itemHosts, bool searchFromTop) {
		    if (aHost == null) return null;
		    if (_itemHosts.Count == 0) return null;
		    int hostIndex = itemHosts.IndexOf(aHost) + 1;
		    if (hostIndex == 0 && !searchFromTop) return null;
	    ...
	    private ListViewItemHost getLeftSideHost(ListViewItemHost aHost, List<ListViewItemHost> itemHosts) {
		    ...
		    if (_itemHosts.Count == 0) return null;
		    int hostIndex = itemHosts.IndexOf(aHost) - 1;
		    if (hostIndex < 0) hostIndex = itemHosts.Count - 1;
		    while (hostIndex >= 0) {
			    if (itemHosts[hostIndex].Visible) {
				    if (itemHosts[hostIndex].Y == aHost.Y) return itemHosts[hostIndex];
			    }
			    hostIndex -= 1;
		    }
		    return null;
	    ...
	    private ListViewItemHost getFirstVisibleHost(List<ListViewItemHost> itemHost) {
		    foreach (ListViewItemHost lviHost in itemHost) {
			    if (lviHost.Visible) return lviHost;
		    }
		    return null;
	    }
	    /// <summary>
	    ...
	    /// </summary>
        private ListViewItemHost getFirstVisibleHost() { return getFirstVisibleHost(""); }
			    ...
			    foreach (ListViewGroupHost lvgHost in _groupHosts) {
				    if (lvgHost.IsVisible && !lvgHost.Group.IsCollapsed) {
					    foreach (ListViewItemHost lviHost in lvgHost.ItemHosts) {
						    if (lviHost.Visible && lviHost.Item.Text.StartsWith(startsWith, StringComparison.OrdinalIgnoreCase)) return lviHost;
					    }
				    }
			    }
	    ...
	    private ListViewItemHost getLastVisibleHost(List<ListViewItemHost> itemHost) {
		    ...
		    while (i >= 0) {
			    if (itemHost[i].Visible) return itemHost[i];
			    i -= 1;
		    }
		    return null;
	    }
	    ...
	    /// </summary>
        private ListViewItemHost getNextVisibleHost(ListViewItemHost aHost) { return getNextVisibleHost(aHost, ""); }
	    private ListViewItemHost getNextVisibleHost(ListViewItemHost aHost, string startsWith) {
		    if (_itemHosts.Count == 0) return null;
		    if (aHost == null) return getFirstVisibleHost(startsWith);
		    if (_showGroups) {
			    int itemIndex = 0;
			    if (aHost.GroupHost != _defaultGroupHost) {
			    ...
			    if (_defaultGroup.IsCollapsed) return null;
	    ...
	    private ListViewItemHost getPrevVisibleHost(ListViewItemHost aHost) {
		    if (_itemHosts.Count == 0) return null;
		    if (aHost == null) return getFirstVisibleHost();
		    if (_showGroups) {
			    int itemIndex = 0;
			    if (object.ReferenceEquals(aHost.GroupHost, _defaultGroupHost)) {
				    if (!_defaultGroup.IsCollapsed) {
					    itemIndex = _defaultGroupHost.ItemHosts.IndexOf(aHost) - 1;
					    while (itemIndex >= 0) {
						    if (_defaultGroupHost.ItemHosts[itemIndex].Visible) return _defaultGroupHost.ItemHosts[itemIndex];
						    itemIndex -= 1;
					    }
				    }
			    }
			    int groupIndex = _groupHosts.IndexOf(aHost.GroupHost);
			    if (groupIndex == -1) groupIndex = _groupHosts.Count - 1;
			    while (groupIndex >= 0) {
				    if (!_groupHosts[groupIndex].Group.IsCollapsed) {
					    itemIndex = _groupHosts[groupIndex].ItemHosts.IndexOf(aHost) - 1;
					    if (itemIndex == -2) itemIndex = _groupHosts[groupIndex].ItemHosts.Count - 1;
					    while (itemIndex >= 0) {
						    if (_groupHosts[groupIndex].ItemHosts[itemIndex].Visible) return _groupHosts[groupIndex].ItemHosts[itemIndex];
						    itemIndex -= 1;
					    }
				    }
				    groupIndex -= 1;
			    ...
			    while (prevIndex >= 0) {
				    if (_itemHosts[prevIndex].Visible) return _itemHosts[prevIndex];
				    prevIndex -= 1;
			    }
		    }
		    return null;
	    ...
	    private void setSelectedHost(ListViewItemHost host) {
		    ...
		    if (_selectedHost == null) selectedHostChanged = true;
		    if (_selectedHost != host) selectedHostChanged = true;
		    //if (_selectedHost != null) _selectedHost.Selected = false;
		    _selectedHost = host;
		    ...
		    if (selectedHostChanged) if (SelectedIndexChanged != null) SelectedIndexChanged(this, new EventArgs());
		    if (_view == Control.View.Preview) drawPreviewHosts();
	    }
        /// <summary>
        /// Sets the selected ListViewItemHost from property to a specified ListViewItemHost.
        /// </summary>
        private void setSelectedHostFromProperty(ListViewItemHost host) {
            bool selectedHostChanged = false;
            if (_selectedHost == null) selectedHostChanged = true;
            if (_selectedHost != host) selectedHostChanged = true;
            if (_selectedHost != null) _selectedHost.Selected = false;
	    ...
	    /// </summary>
	    private void invokeItemHover(ListViewItemHost host) {
            _hoverHost = host;
		    if (ItemHover != null) ItemHover(this, new ItemEventArgs(host.Item));
	    }
	    /// <summary>
		    ...
		    if (aColumn != null) {
			    switch (aColumn.Format) {
				    ...
				    default:
					    // Convert to datetime
					    try {
						    System.DateTime dtValue = (DateTime)value;
						    switch (aColumn.Format) {
	    ...
	    private void showTextBoxEditor(ListViewItemHost aHost) {
		    ...
		    if (!_columns[0].Visible) return;
		    ItemEventArgs beforeLabelEdit = new ItemEventArgs(aHost.Item);
		    if (BeforeLabelEdit != null) BeforeLabelEdit(this, beforeLabelEdit);
		    if (beforeLabelEdit.Cancel) return;
	    ...
	    private Rectangle getRectangle(PointF cPoint, SizeF initSize, int degree, int _maxIncrementWidth, int _maxIncrementHeight) {
		    ...
		    resultSize.Width = initSize.Width + (float)(Math.Sin(degree * Math.PI / 180) * _maxIncrementWidth * Math.Sin(75 * Math.PI / 180));
		    resultSize.Height = initSize.Height + (float)(Math.Sin(degree * Math.PI / 180) * _maxIncrementHeight * Math.Sin(75 * Math.PI / 180));
		    resultRect.X = (int)(cPoint.X - (resultSize.Width / 2));
		    resultRect.Y = (int)(cPoint.Y - resultSize.Height);
		    resultRect.Width = (int)resultSize.Width;
		    resultRect.Height = (int)resultSize.Height;
		    return resultRect;
	    }
	    /// <summary>
	    ...
	    private void drawPreviewHosts() {
		    if (_slideBgImage != null) _slideBgImage.Dispose();
		    if (_itemHosts.Count == 0) return;
		    ...
		    if (_selectedHost != null) frontHost = _selectedHost;
		    else frontHost = getFirstVisibleHost();
		    if (frontHost == null) return;
		    right1 = getNextVisibleHost(frontHost);
		    if (right1 != null) right2 = getNextVisibleHost(right1);
		    left1 = getPrevVisibleHost(frontHost);
		    if (left1 != null) left2 = getPrevVisibleHost(left1);
		    // Drawing preview
		    // Check parameter
		    float maxIncWidth = this.Width / 2;
		    float maxIncHeight = (this.Height - _columnControl.Height) * 3 / 4;
		    ...
		    while (degree <= 90) {
			    if (degree < 90) {
				    ...
				    // Right item
				    if (rightHost != null) {
					    outerPoint1.X = centerPoint.X + (float)(Math.Cos(degree * Math.PI / 180) * (this.Width / 3));
					    outerPoint1.Y = centerPoint.Y + (float)(Math.Sin(degree * Math.PI / 180) * (this.Height / 3) * Math.Cos(75 * Math.PI / 180));
					    rect = getRectangle(outerPoint1, initSize, degree, (int)maxIncWidth, (int)maxIncHeight);
					    rightHost.drawPreviewImage(g, rect, degree);
				    }
				    // Left item
				    if (leftHost != null) {
					    outerPoint2.X = centerPoint.X + (float)(Math.Cos((180 - degree) * Math.PI / 180) * (this.Width / 3));
					    outerPoint2.Y = centerPoint.Y + (float)(Math.Sin((180 - degree) * Math.PI / 180) * (this.Height / 3) * Math.Cos(75 * Math.PI / 180));
					    rect = getRectangle(outerPoint2, initSize, degree, (int)maxIncWidth, (int)maxIncHeight);
					    leftHost.drawPreviewImage(g, rect, degree);
				    }
			    } else {
				    PointF outerPoint = new PointF(0, 0);
				    outerPoint.X = centerPoint.X + (float)(Math.Cos(degree * Math.PI / 180) * (this.Width / 3));
				    outerPoint.Y = centerPoint.Y + (float)(Math.Sin(degree * Math.PI / 180) * (this.Height / 3) * Math.Cos(75 * Math.PI / 180));
				    rect = getRectangle(outerPoint, initSize, degree, (int)maxIncWidth, (int)maxIncHeight);
        ...
        internal void _groupTextAlignChanged(ListViewGroup group) {
            if (_showGroups) this.Invalidate();
        }
            ...
            if (_columnRef > -1) {
                ...
                if (_view == View.Preview) {
                    if (_slideLocation == SlideLocation.Bottom || _slideLocation == SlideLocation.Top) {
                        int x = (int)_slider.Value;
                        foreach (ListViewGroupHost gh in _groupHosts) {
                            gh.X = x;
                            if (gh.IsVisible) x = gh.Right + 5;
                        }
                    } else {
                        int y = (int)_slider.Value;
                        foreach (ListViewGroupHost gh in _groupHosts) {
                            gh.Y = y;
                            if (gh.IsVisible) y = gh.Bottom + 5;
                        }
                    }
                } else {
                    int y = 0;
                    if (_vScroll.Visible) y = -_vScroll.Value;
                    else y = _clientArea.Y;
                    foreach (ListViewGroupHost gh in _groupHosts) {
                        gh.Y = y;
                        if (gh.IsVisible) y = gh.Bottom + 5;
                    }
        ...
        /// <summary>
        /// Routines to change the appearance of a ListViewGroup when collapsed or expanded.
        /// </summary>
        [Description("Routines to change the appearance of a ListViewGroup when collapsed or expanded.")]
        internal void _groupCollapseChanged(ListViewGroup group) {
            if (!_showGroups) return;
            measureAll();
            relocateAll();
            if (!_internalThread) this.Invalidate();
        }
        /// <summary>
        /// Routines to change the check state of a ListViewGroup when Checked property of ListViewGroup has been changed.
        /// </summary>
        ...
        internal void _groupCheckedChanged(ListViewGroup group) {
            ...
            if (!group.Checked) group.CheckedItems.Clear();
        ...
        internal void invokeItemBeforeCheck(ItemEventArgs e) {
            if (ItemBeforeCheck != null) ItemBeforeCheck(this, e);
        }
        internal void _itemTextChanged(ListViewItem item) {
            if (_columns.Count == 0) return;
            if (_columns[0].SizeType == ColumnSizeType.Auto) _columnControl.relocateHosts((_hScroll.Visible ? -_hScroll.Value : 0));
            // Rebuilding filter
            ListViewItemHost itemHost = null;
            List<object> objs = new List<object>();
            foreach (ListViewItemHost lviHost in _itemHosts) {
                if (lviHost.Item == item) itemHost = lviHost;
                if (!objs.Contains(lviHost.Item.Text)) objs.Add(lviHost.Item.Text);
            }
            if (itemHost == null) return;
            _columnControl.reloadFilter(0, objs);
            itemHost.Visible = filterItem(item);
            sortAll();
            if (_view == Control.View.Details || _view == Control.View.List) measureAll();
            relocateAll();
            this.Invalidate();
        }
        ...
        internal void _itemBackColorChanged(ListViewItem item) {
            foreach (ListViewItemHost lviHost in _itemHosts) {
                if (lviHost.Item == item) {
                    if (lviHost.IsVisible) this.Invalidate();
                    return;
                }
            }
        }
        ...
        /// <summary>
        /// Fired when checked property of ListViewItem has been changed.
        /// </summary>
        ...
        internal void _itemCheckedChanged(ListViewItem item) {
            if (_internalThread) {
                if (ItemAfterCheck != null) ItemAfterCheck(this, new ItemEventArgs(item));
                return;
            }
            if (item.Group != null) {
                if (item.Checked) {
            ...
            if (itemHost == null) return;
            if (itemHost.GroupHost != null) itemHost.GroupHost.checkCheckedState();
            changeColumnCheckState();
            if (itemHost.IsVisible) this.Invalidate();
            if (ItemAfterCheck != null) ItemAfterCheck(this, new ItemEventArgs(item));
        }
        internal void _itemUseItemStyleForSubItemsChanged(ListViewItem item) { _itemFontChanged(item); }
        ...
        internal void _subitemValueChanged(ListViewItem.ListViewSubItem subitem) {
            int subItemIndex = subitem.ListViewItem.SubItems.IndexOf(subitem) + 1;
            if (_columns.Count == 0 || subItemIndex >= _columns.Count) return;
            if (_columns[subItemIndex].SizeType == ColumnSizeType.Auto) _columnControl.relocateHosts((_hScroll.Visible ? -_hScroll.Value : 0));
            // Rebuilding filter
            ListViewItemHost itemHost = null;
            List<object> objs = new List<object>();
            foreach (ListViewItemHost lviHost in _itemHosts) {
                if (lviHost.Item == subitem.ListViewItem) itemHost = lviHost;
                if (lviHost.Item.SubItems[subItemIndex] != null) {
                    if (!objs.Contains(lviHost.Item.SubItems[subItemIndex].Value)) objs.Add(lviHost.Item.SubItems[subItemIndex].Value);
                }
            }
            if (itemHost == null) return;
            _columnControl.reloadFilter(subItemIndex, objs);
            itemHost.Visible = filterItem(subitem.ListViewItem);
            if (_view == Control.View.Details) measureAll();
            relocateAll();
            this.Invalidate();
        }
        internal void _subitemFontChanged(ListViewItem.ListViewSubItem subitem) {
            if (subitem.ListViewItem.UseItemStyleForSubItems) return;
            ...
            if (_view == Control.View.Details) {
                if (itemHost.GroupHost != null && _showGroups) {
                    if (itemHost.GroupHost != _defaultGroupHost) {
                        ...
                        while (i < _groupHosts.Count) {
                            _groupHosts[i].Y = y;
                            if (_groupHosts[i].IsVisible) y = _groupHosts[i].Bottom + 5;
                            i = i + 1;
                        }
                        _defaultGroupHost.Y = y;
                    }
        ...
        internal void _subitemBackColorChanged(ListViewItem.ListViewSubItem subitem) {
            if (subitem.ListViewItem.UseItemStyleForSubItems) return;
            _itemBackColorChanged(subitem.ListViewItem);
        }
        internal void _subitemColorChanged(ListViewItem.ListViewSubItem subitem) {
            if (subitem.ListViewItem.UseItemStyleForSubItems) return;
            _itemColorChanged(subitem.ListViewItem);
        }
        internal void _subitemPrintValueOnBarChanged(ListViewItem.ListViewSubItem subitem) {
            _itemColorChanged(subitem.ListViewItem);
        }
        ...
        /// </summary>
        public void CollapseDefaultGroup() { _defaultGroup.collapse(); }
        /// <summary>
        /// Expand the default group of the ListView.
        /// </summary>
        public void ExpandDefaultGroup() { _defaultGroup.expand(); }
        /// <summary>
        ...
        public void ensureVisible(ListViewItem item) {
            ...
            if (aHost == null) return;
            if (!aHost.Visible) return;
            if (_showGroups) aHost.GroupHost.Group.expand();
            int dx = 0;
            int dy = 0;
            if (aHost.X < _clientArea.X || aHost.Right > _clientArea.Right) {
                if (aHost.X < _clientArea.X) dx = aHost.X - _clientArea.X;
                else dx = aHost.Right - _clientArea.Right;
            }
            if (aHost.Y < _clientArea.Y || aHost.Bottom > _clientArea.Bottom) {
                if (aHost.Y < _clientArea.Y) dy = aHost.Y - _clientArea.Y;
                else dy = aHost.Bottom - _clientArea.Bottom;
            }
            ...
            if (_slider.Visible) {
                if (_slider.Orientation == Orientation.Horizontal) _slider.Value += dx;
                else _slider.Value += dy;
            }
        }
        /// <summary>
        ...
        public ListViewItem findItemWithText(string text) {
            foreach (ListViewItem lvi in _items) {
                if (lvi.Text.StartsWith(text, StringComparison.OrdinalIgnoreCase)) return lvi;
            }
            return null;
        }
        /// <summary>
        ...
        public ListViewItem findItemWithText(string text, bool includeSubItem) {
            foreach (ListViewItem lvi in _items) {
                ...
                while (i <= lvi.SubItems.Count) {
                    ...
                    if (strValue.StartsWith(text, StringComparison.OrdinalIgnoreCase)) return lvi;
                    i += 1;
                }
            }
            return null;
        ...
        public ListViewItem findItemWithText(string text, bool includeSubItem, int start) {
            if (start < 0 || start >= _items.Count) return null;
            int index = start;
        ...
        public ListViewItem findItemWithText(string text, bool includeSubItem, int start, bool ignoreCase) {
            ...
            while (index < _items.Count) {
                lvi = _items[index];
                if (lvi.Text.StartsWith(text, strComparison)) return lvi;
                int i = 1;
                while (i <= lvi.SubItems.Count) {
                    string strValue = getValueString(lvi.SubItems[i].Value, i);
                    if (strValue.StartsWith(text, strComparison)) return lvi;
                    i += 1;
                }
                index += 1;
            }
        ...
        public Rectangle getItemRect(int index) {
            if (index >= 0 && index < _items.Count) {
                foreach (ListViewItemHost lviHost in _itemHosts) {
                    if (lviHost.Item == _items[index]) return lviHost.Bounds;
                }
            }
            return new Rectangle(0, 0, 0, 0);
        }
        ...
        public Rectangle getItemRect(ListViewItem item) {
            if (item != null) {
                foreach (ListViewItemHost lviHost in _itemHosts) {
                    if (lviHost.Item == item) return lviHost.Bounds;
                }
            }
            return new Rectangle(0, 0, 0, 0);
        }
        ...
        public void invokeLabelEdit(ListViewItem item) {
            ...
            if (invokedHost == null) return;
            if (!invokedHost.Visible) return;
            ensureVisible(item);
        ...
        public void filterItem(int index, object[] values) {
            ...
            ColumnFilterItem cfi;
		    foreach (ColumnFilterItem cf in filterHandle.Items) cf.Selected = false;
            foreach (object objValue in values) {
			    cfi = filterHandle.getFilterItem(objValue);
			    if (cfi != null) cfi.Selected = true;
		    }
		    filterHandle.FilterMode = FilterMode.ByValue;
		    List<ColumnFilterHandle> handlers = _columnControl.FilterHandlers;
		    foreach (ListViewItemHost lviHost in _itemHosts) lviHost.Visible = filterItem(lviHost.Item, handlers);
		    measureAll();
		    relocateAll();
		    this.Invalidate();
		    if (ColumnFilterChanged != null) ColumnFilterChanged(this, new ColumnEventArgs(_columns[index]));
	    }
        /// <summary>
        ...
        public void filterItem(int index, object maxValue, object minValue, FilterRangeMode mode) {
            if (index < 0 || index >= _columns.Count) return;
            if ((float)maxValue <= (float)minValue) return;
            if (mode != FilterRangeMode.Between || mode != FilterRangeMode.Outside) return;
            ColumnFilterHandle filterHandle = _columnControl.FilterHandler(_columns[index]);
            if (filterHandle == null) return;
        ...
        public void filterItem(int index, string words, FilterRangeMode mode) {
            if (index < 0 || index >= _columns.Count) return;
            if (mode == FilterRangeMode.Between || mode == FilterRangeMode.Outside | mode == FilterRangeMode.Equal) return;
            ColumnFilterHandle filterHandle = _columnControl.FilterHandler(_columns[index]);
            if (filterHandle == null) return;
        ...
        public ListView() {
            ...
            _gObj = Graphics.FromImage(_gBmp);
            ...
            if (this.Font.FontFamily.IsStyleAvailable(FontStyle.Bold)) _groupFont = new Font(this.Font.FontFamily.Name, this.Font.Size, FontStyle.Bold);
            else _groupFont = this.Font;
            ...
            _selectedItems = new SelectedListViewItemCollection(this);
            ...
            _columns.AfterClear += _columns_AfterClear;
            _columns.AfterInsert += _columns_AfterInsert;
            _columns.AfterRemove += _columns_AfterRemove;
            _columnControl.AfterColumnCustomFilter += _columnControl_AfterColumnCustomFilter;
            _columnControl.AfterColumnFilter += _columnControl_AfterColumnFilter;
            _columnControl.AfterColumnResize += _columnControl_AfterColumnResize;
            _columnControl.CheckedChanged += _columnControl_CheckedChanged;
            _columnControl.ColumnOrderChanged += _columnControl_ColumnOrderChanged;
            _groups.AfterClear += _groups_AfterClear;
            _groups.AfterInsert += _groups_AfterInsert;
            _groups.AfterRemove += _groups_AfterRemove;
            _items.AfterClear += _items_AfterClear;
            _items.AfterInsert += _items_AfterInsert;
            _items.AfterRemove += _items_AfterRemove;
            _checkedItems.AfterClear += _checkedItems_AfterClear;
            _checkedItems.AfterRemove += _checkedItems_AfterRemove;
            _frozenItems.AfterClear += _frozenItems_AfterClear;
            _frozenItems.AfterInsert += _frozenItems_AfterInsert;
            _frozenItems.AfterRemove += _frozenItems_AfterRemove;
            _frozenItems.Clearing += _frozenItems_Clearing;
            _frozenItems.Inserting += _frozenItems_Inserting;
            _frozenItems.Removing += _frozenItems_Removing;
            _defaultGroup.AfterCheck += _defaultGroup_AfterCheck;
            _defaultGroup.AfterCollapse += _defaultGroup_AfterCollapse;
            _defaultGroup.AfterExpand += _defaultGroup_AfterExpand;
            _defaultGroup.BeforeCheck += _defaultGroup_BeforeCheck;
            _defaultGroup.BeforeCollapse += _defaultGroup_BeforeCollapse;
            _defaultGroup.BeforeExpand += _defaultGroup_BeforeExpand;
            _slider.ValueChanged += _slider_ValueChanged;
            _txtEditor.KeyDown += _txtEditor_KeyDown;
            _txtEditor.LostFocus += _txtEditor_LostFocus;
            _selectedItems.AfterInsert += _selectedItems_AfterInsert;
            _selectedItems.AfterRemove += _selectedItems_AfterRemove;
            _selectedItems.AfterClear += _selectedItems_AfterClear;
            // Matrixes
        ...
        public new System.Drawing.Color BackColor {
            ...
            set {
                if (base.BackColor != value) {
                    ...
                    if (_view != Control.View.Preview) this.Invalidate();
                }
            }
        }
        // Behavior
        /// <summary>
        /// Gets the collection of ColumnHeader that are assigned to the ListView control.
        /// </summary>
        ...
        public ColumnHeaderCollection Columns { get { return _columns; } }
        /// <summary>
        /// Gets the collection of ListViewGroup that are assigned to the ListView control.
        /// </summary>
        ...
        public ListViewGroupCollection Groups { get { return _groups; } }
        /// <summary>
        /// Gets the collection of ListViewItem that are assigned to the ListView control.
        /// </summary>
        ...
        public ListViewItemCollection Items { get { return _items; } }
        /// <summary>
        /// Gets the collection of checked ListViewItem that are assigned to the ListView control.
        /// </summary>
        ...
        public CheckedListViewItemCollection CheckedItems { get { return _checkedItems; } }
        /// <summary>
        /// Gets the collection of frozen ListViewItem that are assigned to the ListView control.
        /// </summary>
        ...
        public FrozenListViewItemCollection FrozenItems { get { return _frozenItems; } }
        /// <summary>
        /// Determine whether the user can edit the labels of items in the control.
        /// </summary>
        ...
        /// <summary>
        /// Determine whether items are displayed in groups.
        /// </summary>
        ...
        /// <summary>
        /// Determine culture info used to display values.
        /// </summary>
        ...
        public CultureInfo Culture {
            ...
            set {
                if (_ci != value) {
                    if (value == null) _ci = Renderer.Drawing.en_us_ci;
                    else _ci = value;
                    _columnControl.relocateHosts((_hScroll.Visible ? -_hScroll.Value : 0));
                    measureAll();
                    relocateAll();
        ...
        /// <summary>
        /// Determine whether the tooltip is shown when the mouse pointer is hover on a ListViewItem.
        /// </summary>
        [Category("Behavior"), DefaultValue(true), Description("Determine whether the tooltip is shown when the mouse pointer is hover on a ListViewItem.")]
        public bool ItemToolTip {
            get { return _itemTooltip; }
            set { _itemTooltip = value; }
        }
        /// <summary>
        /// Gets or sets a value indicating whether drag bitmap is drawn by your code.
        /// </summary>
        [DefaultValue(false), Category("Behavior"), Description("Gets or sets a value indicating whether drag bitmap is drawn by your code.")]
        public bool CustomDrawDragBitmap {
            get { return _customDrawDragBitmap; }
            set { _customDrawDragBitmap = value; }
        }
        // Appearance
        /// <summary>
        /// Determine a check box appears next to each item in the control.
        /// </summary>
        ...
        /// <summary>
        /// Determine a number appears next to each item in the control.
        /// </summary>
        ...
        /// <summary>
        /// Determine a dropdown options is shown in the column header.
        /// </summary>
        ...
        /// <summary>
        /// Determine whether clicking an item selects all its subitems.
        /// </summary>
        ...
        /// <summary>
        /// Determine how items are displayed in the control.
        /// </summary>
        ...
        public Control.View View {
            get { return _view; }
            set {
                if (value < 0 || value > Control.View.Preview) return;
                if (_view != value) {
                    _view = value;
                    if (_view == View.Preview) AllowDrop = false;
                    else AllowDrop = true;
                    _columnControl.relocateHosts((_hScroll.Visible ? -_hScroll.Value : 0));
                    measureAll();
                    relocateAll();
        ...
        /// <summary>
        /// Determine how items are placed in the control on Preview views.
        /// </summary>
        ...
        public Control.SlideLocation SlideLocation {
            ...
            set {
                if (value < Control.SlideLocation.Top || value > Control.SlideLocation.Bottom) return;
        ...
        /// <summary>
        /// Determine whether multiline item's text and subitem's value is allowed.
        /// </summary>
        ...
        /// <summary>
        /// Determine the size of the images shown in Icon view.
        /// </summary>
        ...
        /// <summary>
        /// Determine the size of the images shown in Thumbnail view.
        /// </summary>
        ...
        [DefaultValue(typeof(System.Drawing.Color), "Black"), Category("Appearance"), Description("")]
        public System.Drawing.Color PreviewBackColor {
        ...
        /// <summary>
        /// Determine which item borders are docked to ListView control.
        /// </summary>
        ...
        public RowsDock FrozenRowsDock {
            ...
            set {
                if (value != RowsDock.Bottom && value != RowsDock.Top) return;
        ...
        // Default Group
        /// <summary>
        /// Determine Text of the default group.
        /// </summary>
        ...
        /// <summary>
        /// Determine TextAlign of the default group.
        /// </summary>
        ...
        /// <summary>
        /// Determine object that contains data about the default group.
        /// </summary>
        ...
        /// <summary>
        /// Determine a value indicating whether the default group is checked.
        /// </summary>
        ...
        public CheckState DefaultGroupCheckState { get { return _defaultGroup.CheckState; } }
        [Category("Default Group"), DefaultValue(false), Description(""), Browsable(false)]
        public bool DefaultGroupIsCollapsed { get { return _defaultGroup.IsCollapsed; } }
        [Category("Default Group"), Browsable(false), Description("")]
        public ListViewItem[] DefaultGroupItems {
            get {
        ...
        public ListViewItem SelectedItem {
            get {
                if (_selectedHost != null) return _selectedHost.Item;
                else return null;
            }
            set {
                if (value == null) {
                ...
                foreach (ListViewItemHost lviHost in _itemHosts) {
                    if (lviHost.Item == value) {
                        setSelectedHostFromProperty(lviHost);
                        if (_selectedHost.Visible) {
                            if (_showGroups && _selectedHost.GroupHost.Group.IsCollapsed) return;
                            // Move the selected host into client area
                            int dx = 0;
                            int dy = 0;
                            if (_selectedHost.X < _clientArea.X || _selectedHost.Bounds.Right > _clientArea.Right) {
                                if (_selectedHost.X > _clientArea.X) dx = _selectedHost.X - _clientArea.X;
                                else dx = _selectedHost.Right - _clientArea.Right;
                            }
                            if (_selectedHost.Y < _clientArea.Y || _selectedHost.Bottom > _clientArea.Bottom) {
                                if (_selectedHost.Y < _clientArea.Y) dy = _selectedHost.Y - _clientArea.Y;
                                else dy = _selectedHost.Bounds.Bottom - _clientArea.Bottom;
                            }
                            if (dx != 0 || dy != 0) {
                                if (_vScroll.Visible) _vScroll.Value += dy;
                                if (_hScroll.Visible) _hScroll.Value += dx;
                                if (_slider.Visible) {
                                    if (_slider.Orientation == Orientation.Horizontal) _slider.Value += dx;
                                    else _slider.Value += dy;
                                }
                ...
                if (_selectedIndex != value) {
                    if (value >= -1 || value < _items.Count - 1) {
                        ...
                        if (value > -1) {
                            ...
                            foreach (ListViewItemHost lviHost in _itemHosts) {
                                if (lviHost.Item == anItem) {
                                    selHost = lviHost;
                                    break; // TODO: might not be correct. Was : Exit For
                                }
                            }
                        }
                        setSelectedHostFromProperty(selHost);
                        int dx = 0;
                        int dy = 0;
                        if (_selectedHost.X < _clientArea.X || _selectedHost.Bounds.Right > _clientArea.Right) {
                            if (_selectedHost.X > _clientArea.X) dx = _selectedHost.X - _clientArea.X;
        ...
        /// <summary>
        /// Gets selected items in ListView.
        /// </summary>
        [Browsable(false), Description("Gets selected items in ListView.")]
        public SelectedListViewItemCollection SelectedItems { get { return _selectedItems; } }
        /// <summary>
        /// Gets the selected indices of the current selected items in ListView.
        /// </summary>
        [Browsable(false), Description("Gets the selected indices of the current selected items in ListView.")]
        public int[] SelectedIndices {
            get {
                List<int> result = new List<int>();
                foreach (ListViewItem lvi in _selectedItems) result.Add(_items.IndexOf(lvi));
                return result.ToArray();
            }
        }
        #endregion
        #region Event Handlers.
        private void _selectedItems_AfterInsert(object sender, CollectionEventArgs e) {
            ListViewItem lvi = (ListViewItem)e.Item;
            bool changed = false;
            foreach (ListViewItemHost lviHost in _itemHosts) {
                if (lviHost.selectHost(lvi, true)) {
                    changed = true;
                    break;
                }
            }
            if (changed && !_internalThread) this.Invalidate();
        }
        private void _selectedItems_AfterRemove(object sender, CollectionEventArgs e) {
            ListViewItem lvi = (ListViewItem)e.Item;
            bool changed = false;
            foreach (ListViewItemHost lviHost in _itemHosts) {
                if (lviHost.selectHost(lvi, false)) {
                    changed = true;
                    break;
                }
            }
            if (_selectedHost != null) {
                if (!_selectedHost.Selected) _selectedHost = null;
                if (_selectedItems.Count > 0) {
                    foreach (ListViewItemHost lviHost in _itemHosts) {
                        if (lviHost.selectHost(_selectedItems[_selectedItems.Count - 1], true)) break;
                    }
                }
            }
            if (changed && !_internalThread) this.Invalidate();
        }
        private void _selectedItems_AfterClear(object semder, CollectionEventArgs e) {
            bool changed = false;
            int i = 0;
            while (i < _itemHosts.Count) {
                _itemHosts[i].selectHost(false);
                i++;
            }
            if (_selectedHost != null) _selectedHost = null;
            if (changed && !_internalThread) this.Invalidate();
        }
        ...
        private void _groups_AfterRemove(object sender, CollectionEventArgs e) {
            ...
            foreach (ListViewGroupHost lvgHost in _groupHosts) {
                if (lvgHost.Group == remGroup) remHost = lvgHost;
            }
            if (remHost == null) return;
            _internalThread = true;
        ...
        private void _hScroll_ValueChanged(object sender, System.EventArgs e) {
            if (!_hScroll.Visible) return;
            if (_view == Control.View.Details) {
                _columnControl.moveColumns(-_hScroll.Value);
                if (_showGroups) {
                    foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.X = _clientArea.X - _hScroll.Value;
                    _defaultGroupHost.X = _clientArea.X - _hScroll.Value;
                } else {
                    foreach (ListViewItemHost lviHost in _itemHosts) lviHost.X = _clientArea.X - _hScroll.Value;
                }
                this.Invalidate(true);
            } else {
                if (_itemHosts.Count == 0) return;
                if (_showGroups) {
                    ...
                    foreach (ListViewItemHost lviHost in _itemHosts) lviHost.X += dx;
                }
                this.Invalidate();
            }
        }
        private void _vScroll_ValueChanged(object sender, System.EventArgs e) {
            ...
            if (_showGroups) {
                ...
                foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.Y += dy;
                _defaultGroupHost.Y += dy;
                if (_txtEditor.Visible) _txtEditor.Top += dy;
            } else {
                int dy = _lastVScrollValue - _vScroll.Value;
                foreach (ListViewItemHost lviHost in _itemHosts) {
                    if (_view == Control.View.Details) {
                        if (!lviHost.Frozen) lviHost.Y += dy;
                    } else {
                        lviHost.Y += dy;
                    }
                }
        ...
        private void _items_AfterInsert(object sender, CollectionEventArgs e) {
            ...
            if (newItem.Group != null) {
                foreach (ListViewGroupHost lvgHost in _groupHosts) {
                    if (lvgHost.Group == newItem.Group) {
                        groupHost = lvgHost;
                        break; // TODO: might not be correct. Was : Exit For
                    }
                }
            ...
            if (_view == Control.View.Preview) drawPreviewHosts();
            this.Invalidate(true);
            if (ItemAdded != null) ItemAdded(this, new ItemEventArgs(newItem));
        }
        private void _items_AfterRemove(object sender, CollectionEventArgs e) {
            ...
            foreach (ListViewItemHost lviHost in _itemHosts) {
                if (lviHost.Item == remItem) {
                    remHost = lviHost;
                    break; // TODO: might not be correct. Was : Exit For
                }
            }
            if (remHost == null) return;
            if (remHost == _selectedHost) setSelectedHost(null);
            ...
            if (_view == Control.View.Preview) drawPreviewHosts();
            this.Invalidate(true);
            if (ItemRemoved != null) ItemRemoved(this, new ItemEventArgs(remItem));
        }
        private void ListView_Disposed(object sender, System.EventArgs e) {
            if (_slideBgImage != null) _slideBgImage.Dispose();
            if (_gObj != null) _gObj.Dispose();
            if (_gBmp != null) _gBmp.Dispose();
        }
        private void ListView_EnabledChanged(object sender, System.EventArgs e) { this.Invalidate(); }
        private void ListView_FontChanged(object sender, System.EventArgs e) {
            if (this.Font.FontFamily.IsStyleAvailable(FontStyle.Bold)) _groupFont = new Font(this.Font.Name, this.Font.Size, FontStyle.Bold);
            else _groupFont = this.Font;
            measureAll();
            relocateAll();
            this.Invalidate();
        }
        private void ListView_GotFocus(object sender, System.EventArgs e) { this.Invalidate(); }
        private void ListView_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e) {
            switch (e.KeyCode) {
                case Keys.Up:
                    if (_itemHosts.Count == 0) return;
                    if (_view == Control.View.Preview && (_slideLocation == Control.SlideLocation.Top || _slideLocation == Control.SlideLocation.Bottom)) return;
                    if (_selectedHost == null) {
                        ...
                        if (_showGroups) {
                            foreach (ListViewGroupHost lvgHost in _groupHosts) {
                                ...
                                if (_selectedHost != null) break; // TODO: might not be correct. Was : Exit For
                            }
                            if (_selectedHost == null) {
                                if (!_defaultGroupHost.IsVisible) return;
                                if (_defaultGroup.IsCollapsed) return;
                                foreach (ListViewItemHost lviHost in _defaultGroupHost.ItemHosts) {
                                    if (lviHost.Visible) {
                                        setSelectedHost(lviHost);
                                        break; // TODO: might not be correct. Was : Exit For
                        ...
                        if (_showGroups) {
                            ...
                            if (upperHost == null) {
                                ...
                                if (groupIndex == -1) {
                                    ...
                                    if (!_groupHosts[groupIndex].Group.IsCollapsed) upperHost = getUpperHost(_selectedHost, _groupHosts[groupIndex].ItemHosts);
                                    groupIndex -= 1;
                                }
                                while (upperHost == null && groupIndex >= 0) {
                                    if (_groupHosts[groupIndex].IsVisible) {
                                        if (!_groupHosts[groupIndex].Group.IsCollapsed) upperHost = getUpperHost(_selectedHost, _groupHosts[groupIndex].ItemHosts, true);
                                    }
                                    groupIndex -= 1;
                                }
                            }
                ...
                case Keys.Down:
                    ...
                    if (_selectedHost == null) {
                        ...
                        if (_showGroups) {
                            if (_selectedHost.GroupHost == _defaultGroupHost) {
                                if (!_defaultGroup.IsCollapsed && _defaultGroupHost.IsVisible) lowerHost = getLowerHost(_selectedHost, _defaultGroupHost.ItemHosts);
                            } else {
                                int groupIndex = _groupHosts.IndexOf(_selectedHost.GroupHost);
                                if (!_groupHosts[groupIndex].Group.IsCollapsed) lowerHost = getLowerHost(_selectedHost, _groupHosts[groupIndex].ItemHosts);
                                groupIndex += 1;
                                while (lowerHost == null && groupIndex < _groupHosts.Count) {
                                    if (_groupHosts[groupIndex].IsVisible) {
                                        if (!_groupHosts[groupIndex].Group.IsCollapsed) lowerHost = getLowerHost(_selectedHost, _groupHosts[groupIndex].ItemHosts, true);
                                    }
                                    groupIndex += 1;
                                }
                                if (lowerHost == null) {
                                    if (!_defaultGroup.IsCollapsed && _defaultGroupHost.IsVisible) lowerHost = getLowerHost(_selectedHost, _defaultGroupHost.ItemHosts, true);
                                }
                            }
                        } else {
                            lowerHost = getLowerHost(_selectedHost, _itemHosts);
                        }
                        if (lowerHost == null) return;
                    ...
                    break;
                case Keys.Left:
                    if (_view == Control.View.Preview && (_slideLocation == Control.SlideLocation.Left || _slideLocation == Control.SlideLocation.Right)) return;
                    if (_view == Control.View.Details) {
                        if (!_hScroll.Visible) return;
                        if (_hScroll.Value > 0) _hScroll.Value -= 1;
                        return;
                    }
                    if (_selectedHost == null) {
                        // There is no selected item, select the first visible item.
                        ...
                        if (_view == Control.View.Preview && (_slideLocation == Control.SlideLocation.Bottom || _slideLocation == Control.SlideLocation.Top)) {
                            if (_showGroups) {
                                if (_selectedHost.GroupHost == _defaultGroupHost) {
                                    if (_defaultGroupHost.IsVisible && !_defaultGroup.IsCollapsed) leftSideHost = getLeftSideHost(_selectedHost, _defaultGroupHost.ItemHosts);
                                } else {
                                    int groupIndex = _groupHosts.IndexOf(_selectedHost.GroupHost);
                                    while (leftSideHost == null && groupIndex >= 0) {
                                        if (_groupHosts[groupIndex].IsVisible && !_groupHosts[groupIndex].Group.IsCollapsed) leftSideHost = getLeftSideHost(_selectedHost, _groupHosts[groupIndex].ItemHosts);
                                        groupIndex -= 1;
                                    }
                                }
                            } else {
                            ...
                            if (_showGroups) {
                                if (_selectedHost.GroupHost.IsVisible && !_selectedHost.GroupHost.Group.IsCollapsed) leftSideHost = getLeftSideHost(_selectedHost, _selectedHost.GroupHost.ItemHosts);
                            } else {
                                leftSideHost = getLeftSideHost(_selectedHost, _itemHosts);
                            }
                        }
                        if (leftSideHost == null) return;
                        setSelectedHost(leftSideHost);
                    }
                    if (_selectedHost.X < _clientArea.X || _selectedHost.Bounds.Right > _clientArea.Right) {
                        int dx = 0;
                        if (_selectedHost.X < _clientArea.X) dx = _selectedHost.X - _clientArea.X;
                        else dx = _selectedHost.Bounds.Right - _clientArea.Right;
                        if (_view == Control.View.Preview) _slider.Value += dx;
                        else _hScroll.Value += dx;
                    } else {
                        this.Invalidate();
                    }
                    break;
                case Keys.Right:
                    ...
                    if (_view == Control.View.Details) {
                        if (!_hScroll.Visible) return;
                        if (_hScroll.Value < _hScroll.Maximum) _hScroll.Value += 1;
                        return;
                    }
                    if (_selectedHost == null) {
                        // There is no selected item, select the first visible item.
                        ...
                        if (_view == Control.View.Preview && (_slideLocation == Control.SlideLocation.Bottom || _slideLocation == Control.SlideLocation.Top)) {
                            if (_showGroups) {
                                if (_selectedHost.GroupHost == _defaultGroupHost) {
                                    if (_defaultGroupHost.IsVisible && !_defaultGroup.IsCollapsed) rightSideHost = getRightSideHost(_selectedHost, _defaultGroupHost.ItemHosts);
                                } else {
                                    int groupIndex = _groupHosts.IndexOf(_selectedHost.GroupHost);
                                    while (rightSideHost == null && groupIndex < _groupHosts.Count) {
                                        if (_groupHosts[groupIndex].IsVisible && !_groupHosts[groupIndex].Group.IsCollapsed) rightSideHost = getRightSideHost(_selectedHost, _groupHosts[groupIndex].ItemHosts);
                                        groupIndex += 1;
                                    }
                            ...
                            if (_showGroups) {
                                if (_selectedHost.GroupHost.IsVisible && !_selectedHost.GroupHost.Group.IsCollapsed) rightSideHost = getRightSideHost(_selectedHost, _selectedHost.GroupHost.ItemHosts);
                            } else {
                                rightSideHost = getRightSideHost(_selectedHost, _itemHosts);
                            }
                        }
                        if (rightSideHost == null) return;
                ...
                case Keys.Home:
                    ...
                    if (_showGroups) {
                        ...
                        if (firstHost == null) {
                            if (_defaultGroupHost.IsVisible && !_defaultGroup.IsCollapsed) firstHost = getFirstVisibleHost(_defaultGroupHost.ItemHosts);
                        }
                    } else {
                ...
                case Keys.End:
                    ...
                    if (_showGroups) {
                        if (_defaultGroupHost.IsVisible && !_defaultGroup.IsCollapsed) lastHost = getLastVisibleHost(_defaultGroupHost.ItemHosts);
                        if (lastHost == null) {
                            int i = _groupHosts.Count - 1;
                            while (i >= 0) {
                                if (_groupHosts[i].IsVisible && !_groupHosts[i].Group.IsCollapsed) lastHost = getLastVisibleHost(_defaultGroupHost.ItemHosts);
                                i -= 1;
                            }
                        }
                    } else {
        ...
        private void ListView_LostFocus(object sender, System.EventArgs e) { this.Invalidate(); }
        private void ListView_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e) {
            ...
            if (!_slider.InterceptMouseDown) {
                if (_showGroups) {
                    foreach (ListViewGroupHost lvgHost in _groupHosts) stateChanged = stateChanged || lvgHost.mouseDown();
                    stateChanged = stateChanged || _defaultGroupHost.mouseDown();
                } else {
                    foreach (ListViewItemHost lviHost in _itemHosts) stateChanged = stateChanged || lviHost.mouseDown();
                }
                if (!stateChanged) setSelectedHost(null);
            }
            this.Invalidate();
            if (_view != View.Preview) {
                if (_selectedHost != null) {
        ...
        private void ListView_MouseEnter(object sender, System.EventArgs e) {
            ...
            if (_vScroll.Visible) {
                ...
                if (_clientArea.Contains(p)) {
                    if (!_mouseOnClientArea) {
                        ...
                        if (_checkBoxes) stateChanged = true;
                    }
                }
            } else {
                if (!_mouseOnClientArea) {
            ...
            if (stateChanged) this.Invalidate();
        }
        private void ListView_MouseLeave(object sender, System.EventArgs e) {
            ...
            if (_showGroups) {
                foreach (ListViewGroupHost lvgHost in _groupHosts) stateChanged = stateChanged || lvgHost.mouseLeave();
                stateChanged = stateChanged || _defaultGroupHost.mouseLeave();
            } else {
                foreach (ListViewItemHost lviHost in _itemHosts) stateChanged = stateChanged || lviHost.mouseLeave();
            }
            if (_slider.Visible) {
                _slider.mouseLeave();
                stateChanged = stateChanged || _slider.Changed;
        ...
        private void ListView_MouseMove(object sender, System.Windows.Forms.MouseEventArgs e) {
            ...
            if (_frozenArea.Contains(e.Location)) {
                if (_showGroups) {
                    foreach (ListViewGroupHost lvgHost in _groupHosts) stateChanged = stateChanged || lvgHost.mouseMove(e);
                    stateChanged = stateChanged || _defaultGroupHost.mouseMove(e);
                } else {
                    foreach (ListViewItemHost lviHost in _itemHosts) stateChanged = stateChanged || lviHost.mouseMove(e);
                }
                if (stateChanged) this.Invalidate();
                return;
            }
            ...
            if (e.Button == MouseButtons.Left && _dragBegin) {
                if (e.Location != _startDragPoint) {
                    if (_selectedHost != null) {
                        if (!_selectedItems.Contains(_selectedHost.Item)) _selectedItems.Add(_selectedHost.Item);
                    }
                    _imglDrag.Images.Clear();
                    Bitmap bmpDrag = new Bitmap(_dragBmpSize, _dragBmpSize);
                    Graphics gDrag = Graphics.FromImage(bmpDrag);
                    if (_customDrawDragBitmap) {
                        if (DrawDragBitmap != null) {
                            PaintEventArgs pe = new PaintEventArgs(gDrag, new Rectangle(0, 0, _dragBmpSize, _dragBmpSize));
                            DrawDragBitmap(this, pe);
                        }
                    } else {
                        gDrag.SmoothingMode = SmoothingMode.AntiAlias;
                        gDrag.TextRenderingHint = System.Drawing.Text.TextRenderingHint.ClearTypeGridFit;
                        gDrag.Clear(Color.Transparent);
                        _selectedHost.drawDrag(gDrag);
                    }
                    if (gDrag != null) gDrag.Dispose();
                    _imglDrag.ImageSize = new Size(_dragBmpSize, _dragBmpSize);
                    _imglDrag.Images.Add(bmpDrag);
                    if (DragHelper.ImageList_BeginDrag(_imglDrag.Handle, 0, (int)(_dragBmpSize * 3 / 4), (int)(_dragBmpSize * 3 / 4))) {
                        _dragSource = _selectedHost.Item;
                        DoDragDrop(_selectedHost.Item, DragDropEffects.All);
                    }
        ...
        private void ListView_MouseUp(object sender, System.Windows.Forms.MouseEventArgs e) {
            ...
            if (!_slider.InterceptMouseDown) {
                if (_showGroups) {
                    foreach (ListViewGroupHost lvgHost in _groupHosts) stateChanged = stateChanged || lvgHost.mouseUp(e);
                    stateChanged = stateChanged || _defaultGroupHost.mouseUp(e);
                } else {
                    foreach (ListViewItemHost lviHost in _itemHosts) stateChanged = stateChanged || lviHost.mouseUp(e);
                }
            }
            if (_slider.Visible) _slider.mouseUp();
            //if (stateChanged) this.Invalidate();
            _internalThread = true;
            ListViewItem selLvi = null;
            if (_selectedHost != null) selLvi = _selectedHost.Item;
            if (System.Windows.Forms.Control.ModifierKeys != Keys.Control) _selectedItems.Clear();
            if (selLvi != null) {
                if (!_selectedItems.Contains(selLvi)) _selectedItems.Add(selLvi);
                else if (System.Windows.Forms.Control.ModifierKeys == Keys.Control) _selectedItems.Remove(selLvi);
                if (_selectedHost != null) _selectedIndex = _items.IndexOf(_selectedHost.Item);
            } else {
        ...
        private void ListView_MouseWheel(object sender, System.Windows.Forms.MouseEventArgs e) {
            if (e.Delta > 0) {
                if (_vScroll.Visible && _vScroll.Value > 0) {
                    if (_vScroll.Value >= _vScroll.SmallChange) _vScroll.Value -= _vScroll.SmallChange;
                    else _vScroll.Value = 0;
                    return;
                }
                if (_hScroll.Visible && _hScroll.Value > 0) {
                    if (_hScroll.Value >= _hScroll.SmallChange) _hScroll.Value -= _hScroll.SmallChange;
                    else _hScroll.Value = 0;
                    return;
                }
                if (_slider.Visible && _slider.Value > 0) {
                    int smallChange = _slider.Maximum / 10;
                    if (_slider.Value >= smallChange) _slider.Value -= smallChange;
                    else _slider.Value = 0;
                    return;
                }
            } else if (e.Delta < 0) {
                if (_vScroll.Visible && _vScroll.Value < _vScroll.Maximum) {
                    if (_vScroll.Value <= _vScroll.Maximum - _vScroll.SmallChange) _vScroll.Value += _vScroll.SmallChange;
                    else _vScroll.Value = _vScroll.Maximum;
                    return;
                }
                if (_hScroll.Visible && _hScroll.Value < _hScroll.Maximum) {
                    if (_hScroll.Value <= _hScroll.Maximum - _hScroll.SmallChange) _hScroll.Value += _hScroll.SmallChange;
                    else _hScroll.Value = _hScroll.Maximum;
                    return;
                }
                if (_slider.Visible && _slider.Value < _slider.Maximum) {
                    int smallChange = _slider.Maximum / 10;
                    if (_slider.Value <= _slider.Maximum - smallChange) _slider.Value += smallChange;
                    else _slider.Value = _slider.Maximum;
                    return;
                }
            }
        }
        private void ListView_Paint(object sender, System.Windows.Forms.PaintEventArgs e) {
            ...
            e.Graphics.InterpolationMode = InterpolationMode.HighQualityBicubic;
            e.Graphics.TextRenderingHint = System.Drawing.Text.TextRenderingHint.ClearTypeGridFit;
            if (_view == Control.View.Preview) e.Graphics.Clear(Color.Black);
            ...
            if (_view == Control.View.Details) {
                ...
                foreach (ColumnHeader ch in unfrozenCols) {
                    ...
                    if (ColumnBackgroundPaint != null) ColumnBackgroundPaint(this, pe);
                    e.Graphics.DrawLine(linePen, colRect.Right, colRect.Y, colRect.Right, colRect.Bottom);
                }
                foreach (ListViewItemHost lviHost in _itemHosts) {
                    if (!lviHost.Frozen) lviHost.drawUnFrozen(e.Graphics, frozenCols.Count);
                }
                if (frozenCols.Count > 0) {
                    ...
                    foreach (ListViewItemHost lviHost in _itemHosts) {
                        if (!lviHost.Frozen) lviHost.drawFrozen(e.Graphics, unfrozenCols.Count);
                    }
                }
                if (_showGroups) {
                    foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.draw(e.Graphics, _checkBoxes);
                ...
                if (_frozenArea.Height > 0) {
                    ...
                    if (_frozenRowsDock == RowsDock.Top) e.Graphics.DrawLine(Pens.LightGray, 0, _frozenArea.Bottom - 1, this.Width, _frozenArea.Bottom - 1);
                    else e.Graphics.DrawLine(Pens.LightGray, 0, _frozenArea.Y, this.Width, _frozenArea.Y);
                    // Draw the items
                    foreach (ColumnHeader ch in unfrozenCols) {
                        colRect = _columnControl.ColumnRectangle(ch);
                        colRect.Y = _clientArea.Y;
                    ...
                    foreach (ListViewItemHost lviHost in _frozenHosts) lviHost.drawUnFrozen(e.Graphics, frozenCols.Count);
                    if (frozenCols.Count > 0) {
                        colRect = _columnControl.FrozenRectangle;
                        colRect.Y = 0;
                        colRect.Height = this.Height;
                        ...
                        foreach (ListViewItemHost lviHost in _frozenHosts) lviHost.drawFrozen(e.Graphics, unfrozenCols.Count);
                    }
                ...
                if (_checkBoxes || _rowNumbers) {
                    ...
                    if (_checkBoxes) aRect.Width = 22;
                    if (_rowNumbers) aRect.Width += (int)(e.Graphics.MeasureString(_items.Count.ToString(), Font).Width + 2);
                    int rowNumbersX = 1;
                    LinearGradientBrush rectBrush = new LinearGradientBrush(aRect, Color.Black, Color.White, LinearGradientMode.Horizontal);
                    rectBrush.InterpolationColors = Ai.Renderer.Column.NormalBlend();
                    e.Graphics.FillRectangle(rectBrush, aRect);
                    if (_checkBoxes) {
                        rowNumbersX = 23;
                        if (_showGroups) {
                            foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.drawItemsCheckBox(e.Graphics);
                            _defaultGroupHost.drawItemsCheckBox(e.Graphics);
                        } else {
                            foreach (ListViewItemHost lviHost in _itemHosts) {
                                if (lviHost.IsVisible) lviHost.drawCheckBox(e.Graphics);
                            }
                        }
                    }
                    if (_rowNumbers) {
                        ...
                        if (_showGroups) {
                            foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.drawItemsRowNumber(e.Graphics, rowFormat, rowNumbersX, rowWidth, (Enabled ? Brushes.Black : Brushes.Gray));
                            ...
                            foreach (ListViewItemHost lviHost in _itemHosts) {
                                if (lviHost.IsVisible) {
                                    ...
                                    e.Graphics.DrawString(lviHost.RowIndex.ToString(), this.Font, (Enabled ? Brushes.Black : Brushes.Gray), rowRect, rowFormat);
                                }
                            }
                        }
                        rowFormat.Dispose();
                    ...
                    e.Graphics.FillRectangle(rectBrush, aRect);
                    rectBrush.Dispose();
                    e.Graphics.DrawLine(Ai.Renderer.Column.NormalBorderPen(), aRect.Right - 1, 0, aRect.Right - 1, this.Height);
                }
            } else {
                if (_view == Control.View.Preview) {
                    if (_slideBgImage != null) e.Graphics.DrawImage(_slideBgImage, 0, _columnControl.Bottom);
                    ...
                    switch (_slideLocation) {
                        ...
                        case Control.SlideLocation.Left:
                            ...
                            slideBrush = new LinearGradientBrush(slideRect, Color.Black, Color.FromArgb(127, 0, 0, 0), (float)0);
                            slideRect.Width -= 1;
                            break;
                ...
                if (_showGroups) {
                    foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.draw(e.Graphics, _mouseOnClientArea && _checkBoxes);
            ...
            if (_slider.Visible) _slider.draw(e.Graphics);
            ...
            if (_columnControl.OnColumnResize) e.Graphics.DrawLine(Pens.Black, _columnControl.ResizeCurrentX, 0, _columnControl.ResizeCurrentX, this.Height);
        }
        ...
        private void ListView_DragEnter(object sender, DragEventArgs e) {
            DragHelper.ImageList_DragEnter(IntPtr.Zero, e.X - this.Left,
                e.Y - this.Top);
            _dragKeyState = e.KeyState;
            if (_dragBegin) _dragSource = _selectedItems;
            else _dragSource = e.Data;
            ItemDragDropEventArgs dde = new ItemDragDropEventArgs(_dragSource, null, null, e.KeyState);
            if (ItemDragStart != null) ItemDragStart(this, dde);
            e.Effect = dde.Effects;
        }
        private void ListView_DragLeave(object sender, EventArgs e) {
            DragHelper.ImageList_DragLeave(this.Handle);
            if (ItemDragLeave != null) {
                ItemDragDropEventArgs dde = new ItemDragDropEventArgs(_dragSource, null, null, _dragKeyState);
                ItemDragLeave(this, dde);
            }
        }
        private void ListView_DragOver(object sender, DragEventArgs e) {
            Point formP = System.Windows.Forms.Control.MousePosition;
            Point ce = this.PointToClient(formP);
            DragHelper.ImageList_DragMove(formP.X, formP.Y);
            if (ce.Y > this.Height - 20 || ce.Y < (_columnControl.Visible ? _columnControl.Height : 0) + 20) {
                if (!_timer.Enabled) _timer.Enabled = _vScroll.Visible;
            }
            else _timer.Enabled = false;
            bool stateChanged = false;
            bool xRet = false;
            MouseEventArgs me = new MouseEventArgs(MouseButtons.Left, 0, ce.X, ce.Y, 0);
            if (_frozenArea.Contains(me.Location)) {
                if (_showGroups) {
                    foreach (ListViewGroupHost lvgHost in _groupHosts) stateChanged = stateChanged || lvgHost.mouseLeave();
                    stateChanged = stateChanged || _defaultGroupHost.mouseLeave();
                } else {
            ...
            if (!xRet) {
                if (_vScroll.Visible) {
                    if (_clientArea.Contains(me.Location)) {
                        if (!_mouseOnClientArea) {
                            _mouseOnClientArea = true;
                            if (_checkBoxes) stateChanged = true;
                        }
                ...
                if (!_slider.InterceptMouseDown) {
                    if (_showGroups) {
                        foreach (ListViewGroupHost lvgHost in _groupHosts) stateChanged = stateChanged || lvgHost.mouseMove(me);
                        stateChanged = stateChanged || _defaultGroupHost.mouseMove(me);
                    } else {
                        foreach (ListViewItemHost lviHost in _itemHosts) stateChanged = stateChanged | lviHost.mouseMove(me);
                    }
                }
                if (_slider.Visible) {
                    _slider.mouseMove(me.Location);
                    stateChanged = stateChanged || _slider.Changed;
                }
            }
            if (stateChanged) this.Invalidate();
            if (ItemDragMove != null) {
                ItemDragDropEventArgs dde;
                if (_hoverHost != null) dde = new ItemDragDropEventArgs(_dragSource, null, _hoverHost.Item, e.KeyState);
                else dde = new ItemDragDropEventArgs(_dragSource, null, null, e.KeyState);
                ItemDragMove(this, dde);
                e.Effect = dde.Effects;
            }
        }
        private void ListView_DragDrop(object sender, DragEventArgs e) {
            bool stateChanged = false;
            Point cp = this.PointToClient(System.Windows.Forms.Control.MousePosition);
            MouseEventArgs me = new MouseEventArgs(MouseButtons.Left, 0, cp.X, cp.Y, 0);
            foreach (ListViewItemHost tnHost in _itemHosts) {
                if (tnHost.Visible) stateChanged = stateChanged || tnHost.mouseUp(me);
            }
            if (stateChanged) this.Invalidate();
            if (ItemDragDrop != null) {
                ItemDragDropEventArgs dde;
                if (_hoverHost != null) dde = new ItemDragDropEventArgs(_dragSource, _hoverHost.Item, _hoverHost.Item, e.KeyState);
                else dde = new ItemDragDropEventArgs(_dragSource, null, null, e.KeyState);
                ItemDragDrop(this, dde);
            }
            _dragBegin = false;
            _dragKeyState = 0;
            _timer.Enabled = false;
        ...
        private void _timer_Tick(object sender, EventArgs e) {
            Point ce = this.PointToClient(System.Windows.Forms.Control.MousePosition);
            if (_clientArea.Contains(ce)) {
                if (ce.Y > this.Height - 20) {
                    if (_vScroll.Value < _vScroll.Maximum) {
                        if (_vScroll.Value + _vScroll.SmallChange < _vScroll.Maximum) _vScroll.Value += _vScroll.SmallChange;
                        else _vScroll.Value = _vScroll.Maximum;
                    }
                } else {
                    if (_vScroll.Value > _vScroll.Minimum) {
                        if (_vScroll.Value - _vScroll.SmallChange > _vScroll.Minimum) _vScroll.Value -= _vScroll.SmallChange;
                        else _vScroll.Value = _vScroll.Minimum;
                    }
                }
            }
        }
        private void _checkedItems_AfterClear(object sender, CollectionEventArgs e) {
            foreach (ListViewItem item in _items) item.Checked = false;
        }
        ...
        private void _columnControl_AfterColumnCustomFilter(object sender, ColumnEventArgs e) {
            ...
            if (ColumnCustomFilter != null) ColumnCustomFilter(this, cEvent);
            if (!cEvent.CancelFilter) {
                ...
                relocateAll();
                this.Invalidate();
                if (ColumnFilterChanged != null) ColumnFilterChanged(this, e);
            }
        }
        ...
        private void _columnControl_AfterColumnResize(object sender, ColumnEventArgs e) {
            ...
            relocateAll();
            this.Invalidate();
            if (ColumnSizeChanged != null) ColumnSizeChanged(this, e);
        }
        private void _columnControl_CheckedChanged(object sender, System.EventArgs e) {
            ...
            foreach (ListViewItem lvi in _items) {
                ...
                if (lvi.Group != null) {
                    if (lvi.Checked) lvi.Group.CheckedItems.Add(lvi);
                    else lvi.Group.CheckedItems.Remove(lvi);
                }
            }
            foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.checkCheckedState();
        ...
        private void _columnControl_ColumnOrderChanged(object sender, ColumnEventArgs e) {
            ...
            if (ColumnOrderChanged != null) ColumnOrderChanged(this, e);
        }
        private void _columns_AfterClear(object sender, CollectionEventArgs e) { _hScroll.Visible = false; }
        ...
        private void _slider_ValueChanged(object sender, System.EventArgs e) {
            ...
            if (_slider.Orientation == Orientation.Horizontal) {
                if (_showGroups) {
                    ...
                    if (_groupHosts.Count > 0) lastX = _groupHosts[0].X;
                    int dx = (int)((_clientArea.X - _slider.Value) - lastX);
                    foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.X += dx;
                ...
                if (_showGroups) {
                    ...
                    if (_groupHosts.Count > 0) lastY = _groupHosts[0].Y;
                    int dy = (int)((_clientArea.Y - _slider.Value) - lastY);
                    foreach (ListViewGroupHost lvgHost in _groupHosts) lvgHost.Y += dy;
                    _defaultGroupHost.Y += dy;
                } else {
                    int lastY = _itemHosts[0].Y;
                    int dy = (int)((_clientArea.Y - _slider.Value) - lastY);
                    foreach (ListViewItemHost lviHost in _itemHosts) lviHost.Y += dy;
                }
            }
            this.Invalidate();
        }
        private void _defaultGroup_AfterCheck(object sender, GroupEventArgs e) {
            if (DefaultGroupAfterCheck != null) DefaultGroupAfterCheck(this, e);
        }
        private void _defaultGroup_AfterCollapse(object sender, GroupEventArgs e) {
            if (DefaultGroupAfterCollape != null) DefaultGroupAfterCollape(this, e);
        }
        private void _defaultGroup_AfterExpand(object sender, GroupEventArgs e) {
            if (DefaultGroupAfterExpand != null) DefaultGroupAfterExpand(this, e);
        }
        private void _defaultGroup_BeforeCheck(object sender, GroupEventArgs e) {
            if (DefaultGroupBeforeCheck != null) DefaultGroupBeforeCheck(this, e);
        }
        private void _defaultGroup_BeforeCollapse(object sender, GroupEventArgs e) {
            if (DefaultGroupBeforeCollapse != null) DefaultGroupBeforeCollapse(this, e);
        }
        private void _defaultGroup_BeforeExpand(object sender, GroupEventArgs e) {
            if (DefaultGroupBeforeExpand != null) DefaultGroupBeforeExpand(this, e);
        }
        private void _tooltip_Draw(object sender, DrawEventArgs e) {
            Ai.Renderer.ToolTip.drawToolTip(e.Graphics, new Rectangle((int)e.Rectangle.X, (int)e.Rectangle.Y, (int)e.Rectangle.Width, (int)e.Rectangle.Height), _currentToolTipTitle, _currentToolTip, _currentToolTipImage);
            _currentToolTipTitle = "";
                    ...
                    if (!_txtEditor.Multiline) {
                        if (_currentEditedHost != null) {
                            if (_currentEditedHost.Item.Text != _txtEditor.Text) {
                                ...
                                if (AfterLabelEdit != null) AfterLabelEdit(this, new ItemEventArgs(_currentEditedHost.Item));
                            }
                        }
                        this.Focus();
                    }
        ...
        private void _txtEditor_LostFocus(object sender, System.EventArgs e) { _txtEditor.Visible = false; }
        ...
        private void _frozenItems_Clearing(object sender, CollectionEventArgs e) {
            foreach (ListViewItemHost lviHost in _itemHosts) {
                foreach (ListViewItem lvi in _frozenItems) {
                    if (lviHost.Item == lvi) lviHost.Frozen = false;
                }
            }
            _frozenHosts.Clear();
        }
        ...
        private void _frozenItems_Removing(object sender, CollectionEventArgs e) {
            ...
            foreach (ListViewItemHost lviHost in _itemHosts) {
                if (lviHost.Item == remFrozen) {
                    lviHost.Frozen = false;
                    if (_frozenHosts.Contains(lviHost)) _frozenHosts.Remove(lviHost);
                    return;
                }
            }
        }
    ...
    public enum View {
        ...
        /// <summary>
        /// Show full sized image on slide show.
        /// </summary>
        [Description("Show full sized image on slide show.")]
        Preview = 5
    }
    /// <summary>
    /// Specifies the position and manner in which the slider should be shown in Preview view mode.
    /// </summary>
